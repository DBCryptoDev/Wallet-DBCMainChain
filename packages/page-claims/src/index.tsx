...
function ClaimsApp({ basePath }: Props): React.ReactElement<Props> {
  ...
  const [preclaimEthereumAddress, setPreclaimEthereumAddress] = useState<
    string | null | undefined | typeof PRECLAIMS_LOADING
  >(PRECLAIMS_LOADING);
  ...
  const statementKind = useCall<StatementKind | null>(
    !isPreclaimed && !isOldClaimProcess && !!ethereumAddress && api.query.claims.signing,
    [ethereumAddress],
    transformStatement
  );
      ...
      <Columar>
        <Columar.Column>
          <Card withBottomMargin>
            <h3>
              {t<string>("1. Select your {{chain}} account", {
                replace: {
                  chain: systemChain,
                },
              })}
            ...
            {step === Step.Account && (
              <Button.Group>
                <Button
                  ...
                  label={preclaimEthereumAddress === PRECLAIMS_LOADING ? t<string>("Loading") : t<string>("Continue")}
          ...
          {step >= Step.Sign && !isPreclaimed && (
            <Card>
              <h3>
                {t<string>("{{step}}. Sign with your ETH address", {
                  replace: { step: isOldClaimProcess ? "2" : "3" },
                })}
              </h3>
              {!isOldClaimProcess && <Statement kind={statementKind} systemChain={systemChain} />}
              <div>
                {t<string>(
              ...
              <CopyToClipboard onCopy={onCopy} text={payload}>
                <Payload data-for="tx-payload" data-tip>
                  {payload}
                </Payload>
              </CopyToClipboard>
              <Tooltip
