...
export async function multiProposeBounty(
  api: ApiPromise,
  ...
  const arr = Array.from({ length: numberOfBounties }, (_, i) =>
    api.tx.bounties.proposeBounty(new BN(500_000_000_000_000), `new bounty no ${i}`)
  );

  await execute(api.tx.utility.batch(arr), signer);
...
export async function multiApproveBounty(api: ApiPromise, bountyIndexes: number[], signer: KeyringPair): Promise<void> {
  ...
  await multiAcceptMotion(
    api,
    extractHashesFromProposals(bountyProposals),
    extractIndexesFromProposals(bountyProposals)
  );
}
...
export async function multiProposeCurator(
  api: ApiPromise,
  bountyIndexes: number[],
  signer: KeyringPair
): Promise<void> {
