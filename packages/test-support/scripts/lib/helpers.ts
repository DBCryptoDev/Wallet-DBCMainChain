...
export async function getMotion(api: ApiPromise, index: number): Promise<DeriveCollectiveProposal> {
  ...
  const bountyProposals = bounties.find((bounty) => bounty.index.toNumber() === index)
    ?.proposals as DeriveCollectiveProposal[];

  return bountyProposals[0];
}

export async function multiProposeMotion(
  api: ApiPromise,
  submittableExtrinsicArray: SubmittableExtrinsic<"promise">[],
  signer: KeyringPair
): Promise<void> {
  const proposeExtrinsicArray = submittableExtrinsicArray.map((extrinsic) =>
...
export async function multiGetMotion(api: ApiPromise, indexes: number[]): Promise<DeriveCollectiveProposal[]> {
  const bounties = await api.derive.bounties.bounties();
  const bountyProposals = indexes.map(
    (index) => bounties.find((bounty) => bounty.index.toNumber() === index)?.proposals as DeriveCollectiveProposal[]
  );
...
async function multiVoteAye(acceptMotionSigners: KeyringPair[], api: ApiPromise, indexes: number[], hashes: Hash[]) {
  await Promise.all(
    acceptMotionSigners.map((signer) =>
      execute(
        api.tx.utility.batch(indexes.map((bountyIndex, i) => api.tx.council.vote(hashes[i], bountyIndex, true))),
        signer
      )
    )
  );
