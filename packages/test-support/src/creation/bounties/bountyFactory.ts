...
export class BountyFactory {
  ...
  public aBountyIndex = (index = 0): BountyIndex => this.#registry.createType("BountyIndex", index);

  public defaultBounty = (): Bounty => this.#registry.createType("Bounty");

  public aBountyStatus = (status: string): BountyStatus => this.#registry.createType("BountyStatus", status);

  public bountyStatusWith = ({
  ...
  public aBounty = ({
    fee = balanceOf(10),
    status = this.aBountyStatus("Proposed"),
    value = balanceOf(500),
  }: Partial<Bounty> = {}): Bounty => this.#registry.createType("Bounty", { fee, status, value });
}
