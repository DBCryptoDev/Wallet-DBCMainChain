  ...
  return (
    <SummaryBox>
      <section>
        ...
        {health && (
          <>
            <CardSummary className="media--800" label={t<string>("total peers")}>
              {formatNumber(health.peers)}
            </CardSummary>
            <CardSummary className="media--800" label={t<string>("syncing")}>
      ...
      {extrinsics && extrinsics.length > 0 && (
        <section className="media--1200">
          <CardSummary label={t<string>("queued tx")}>{extrinsics.length}</CardSummary>
        </section>
      )}
      <section>
        {peerBest?.gtn(0) && <CardSummary label={t<string>("peer best")}>{formatNumber(peerBest)}</CardSummary>}
