...
function transformResult([events, getBlock, getHeader]: [EventRecord[], SignedBlock, HeaderExtended?]): [
  KeyedEvent[],
  SignedBlock,
  HeaderExtended?
] {
...
function BlockByHash({ className = "", error, value }: Props): React.ReactElement<Props> {
  ...
  useEffect((): void => {
    value &&
      Promise.all([api.query.system.events.at(value), api.rpc.chain.getBlock(value), api.derive.chain.getHeader(value)])
        .then((result): void => {
    ...
    <div className={className}>
      ...
      >
        {myError ? (
          <tr>
            <td colSpan={6}>
              {t("Unable to retrieve the specified block details. {{error}}", { replace: { error: myError.message } })}
            </td>
          </tr>
        ) : (
          getBlock &&
      ...
      {getBlock && getHeader && (
        <>
          <Extrinsics blockNumber={blockNumber} events={events} value={getBlock.block.extrinsics} />
          <Columar>
            <Columar.Column>
              <Events
