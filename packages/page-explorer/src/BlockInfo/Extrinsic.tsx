...
function ExtrinsicDisplay({ blockNumber, className = "", events, index, value }: Props): React.ReactElement<Props> {
  ...
  const { meta, method, section } = useMemo(() => value.registry.findMetaCall(value.callIndex), [value]);
  ...
  const thisEvents = useMemo(() => filterEvents(index, events), [index, events]);

  return (
    <tr className={className} key={`extrinsic:${index}`}>
      <td className="top" colSpan={2}>
        <Expander summary={`${section}.${method}`} summaryMeta={meta}>
          <Call
            className="details"
            mortality={mortality}
            tip={value.tip?.toBn()}
      ...
      <td className="top" colSpan={2}>
        {thisEvents.map(({ key, record }) => (
          <Event className="explorer--BlockByHash-event" key={key} value={record} />
        ))}
      </td>
      <td className="top">
        {value.isSigned && (
