...
function formatU8a(value: Raw): React.ReactNode {
  return <Params isDisabled params={[{ type: getTypeDef("Bytes") }]} values={[{ isValid: true, value }]} />;
}
...
function Logs({ value }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <Table empty={t<string>("No logs available")} header={headerRef.current}>
      {value?.map((log, index) => (
        <tr key={`log:${index}`}>
          <td className="overflow">
            <Expander summary={log.type.toString()}>{formatItem(log)}</Expander>
          </td>
        </tr>
      ))}
    </Table>
