...
function areParamsValid(
  ...
  return values.reduce(
    (isValid: boolean, value): boolean => {
      return isValid && !isUndefined(value) && !isUndefined(value.value) && value.isValid;
    },
    type.isDoubleMap ? values.length === 2 : values.length === (type.isMap ? 1 : 0)
  );
}
...
function expandKey(api: ApiPromise, key: QueryableStorageEntry<"promise">): KeyState {
  ...
  return {
    defaultValues:
      section === "session" && type.isDoubleMap
