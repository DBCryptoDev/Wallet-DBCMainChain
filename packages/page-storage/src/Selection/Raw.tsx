...
function Raw({ onAdd }: Props): React.ReactElement<Props> {
  ...
  const [{ isValid, key }, setValue] = useState<{ isValid: boolean; key: Uint8Array }>({
    isValid: false,
    key: new Uint8Array([]),
  });
  ...
  const _onChangeKey = useCallback((key: string): void => {
  ...
  return (
    <section className="storage--actionrow">
      <div className="storage--actionrow-value">
        <Input autoFocus label={t<string>("hex-encoded storage key")} onChange={_onChangeKey} onEnter={_onAdd} />
      </div>
      <div className="storage--actionrow-buttons">
        <Button icon="plus" isDisabled={!isValid} onClick={_onAdd} />
      </div>
    </section>
  );
}
