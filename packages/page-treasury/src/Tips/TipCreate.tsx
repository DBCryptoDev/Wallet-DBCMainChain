...
function TipCreate({ members }: Props): React.ReactElement<Props> {
  ...
  return (
    <>
      <Button icon="plus" label={t<string>("Propose tip")} onClick={toggleOpen} />
      {isOpen && (
        <Modal header={t<string>("Submit tip request")} size="large">
          <Modal.Content>
            <Modal.Columns
              hint={t<string>("Use this account to request the tip from. This can be a normal or council account.")}
            >
              <InputAddress
            ...
            <Modal.Columns
              hint={t<string>("A reason (to be stored-on-chain) as to why the recipient deserves a tip payout.")}
            >
              <Input
                autoFocus
