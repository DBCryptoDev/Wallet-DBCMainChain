...
function Tip({
  ...
  return (
    <tr className={className}>
      ...
      <td className="address media--1400">{finder && <AddressMini value={finder} />}</td>
        ...
        {tips.length !== 0 && (
          ...
          >
            {tips.map(([tipper, balance]) => (
              <AddressMini balance={balance} key={tipper.toString()} value={tipper} withBalance />
            ))}
          </Expander>
        )}
      </td>
          ...
          ? bestNumber &&
            closesAt.gt(bestNumber) && (
              <div className="closingTimer">
                <BlockToTime value={closesAt.sub(bestNumber)} />#{formatNumber(closesAt)}
              </div>
            )
          : finder &&
            isFinder && (
      ...
      <td className="badge media--1700">{isMember && <Icon color={isTipper ? "green" : "gray"} icon="asterisk" />}</td>
      <td>
        <Checkbox isDisabled={!isMember} onChange={setMedianTip} value={isMedianSelected} />
      </td>
      <td className="links media--1700">
        <LinkExternal data={hash} isLogo type="tip" />
      </td>
    </tr>
  );
}
