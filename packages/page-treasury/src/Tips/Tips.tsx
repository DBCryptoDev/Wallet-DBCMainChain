...
function Tips({ className = "", defaultId, hashes, isMember, members, onSelectTip }: Props): React.ReactElement<Props> {
  ...
  const tipsWithHashes = useCall<[[string[]], Option<OpenTip>[]]>(
    hashes && (api.query.tips || api.query.treasury).tips.multi,
    [hashes],
    TIP_OPTS
  );
      ...
      filter={
        isMember && (
          <div className="tipsFilter">
            <Toggle label={t<string>("show only untipped/closing")} onChange={setOnlyUntipped} value={onlyUntipped} />
          </div>
        )
      }
