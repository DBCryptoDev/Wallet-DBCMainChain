...
function ProposalDisplay({
  ...
  isMember,
  members,
  proposal: { council, id, proposal },
  withSend,
}: Props): React.ReactElement<Props> {
  const { t } = useTranslation();

  const hasProposals = useMemo(
    () => !!council.map(({ votes }) => (votes ? votes.index.toNumber() : -1)).filter((index) => index !== -1).length,
    [council]
  );
    ...
    <tr className={className}>
      ...
      <td className={hasProposals ? "middle" : "button"}>
        {hasProposals ? (
          ...
          withSend && <Council id={id} isDisabled={!isMember} members={members} />
        )}
      </td>
      <td className="links">
        <LinkExternal data={id} isLogo type="treasury" />
      </td>
    </tr>
  );
}
