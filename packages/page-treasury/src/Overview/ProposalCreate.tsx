...
function Propose({ className }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <>
      {isOpen && (
        <Modal className={className} header={t<string>("Submit treasury proposal")} size="large">
          <Modal.Content>
            ...
            <Modal.Columns
              hint={
                <>
                  ...
                  <p>
                    {t<string>(
                      "Of the beneficiary amount, at least {{bondPercentage}} would need to be put up as collateral. The maximum of this and the minimum bond will be used to secure the proposal, refundable if it passes.",
                      { replace: { bondPercentage } }
                    )}
                  </p>
                </>
              }
            >
              <InputBalance
                ...
                onChange={setValue}
              />
              <Static help={t<string>("The on-chain percentage for the treasury")} label={t<string>("proposal bond")}>
                {bondPercentage}
              </Static>
              <InputBalance
              ...
              <MarkWarning
                content={t<string>(
                  "Be aware that once submitted the proposal will be put to a council vote. If the proposal is rejected due to a lack of info, invalid requirements or non-benefit to the network as a whole, the full bond posted (as describe above) will be lost."
                )}
              />
            </Modal.Columns>
          </Modal.Content>
      ...
      <Button icon="plus" label={t<string>("Submit proposal")} onClick={toggleOpen} />
    </>
  );
}
