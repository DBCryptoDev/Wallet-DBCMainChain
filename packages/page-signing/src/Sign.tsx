...
function Sign({ className = "" }: Props): React.ReactElement<Props> {
  ...
  const _onChangeData = useCallback((data: string) => setData({ data, isHexData: isHex(data) }), []);
      ...
      <div className="toolbox--Sign-input">
        ...
        <div className="ui--row">
          <Static
            ...
            value={isHexData ? t<string>("Yes") : t<string>("No")}
          />
        </div>
        <div className="ui--row">
          <Output
        ...
        <div className="unlock-overlay" hidden={!isUsable || !isLocked || isInjected}>
          {isLocked && (
            <div className="unlock-overlay-warning">
              <div className="unlock-overlay-content">
                {t<string>("You need to unlock this account to be able to sign data.")}
                <br />
                <Button.Group>
                  <Button icon="unlock" label={t<string>("Unlock account")} onClick={toggleUnlock} />
                </Button.Group>
              </div>
            </div>
          )}
        </div>
        <div className="unlock-overlay" hidden={isUsable}>
          <div className="unlock-overlay-warning">
            <div className="unlock-overlay-content">
              {isInjected
                ? t<string>(
        ...
        {isUnlockVisible && <Unlock onClose={toggleUnlock} onUnlock={_onUnlock} pair={currentPair} />}
      </div>
      <Button.Group>
        <Button icon="key" isDisabled={!(isUsable && !isLocked)} label={t<string>("Sign message")} onClick={_onSign} />
      </Button.Group>
    </div>
  );
}
