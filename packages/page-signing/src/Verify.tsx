...
function Verify({ className = "" }: Props): React.ReactElement {
  ...
  const [{ cryptoType, isValid }, setValidity] = useState<{ cryptoType: CryptoTypes; isValid: boolean }>({
    cryptoType: "unknown",
    isValid: false,
  });
  const [{ data, isHexData }, setData] = useState<{ data: string; isHexData: boolean }>({ data: "", isHexData: false });
  const [{ isValidPk, publicKey }, setPublicKey] = useState<{ isValidPk: boolean; publicKey: Uint8Array | null }>({
    isValidPk: false,
    publicKey: null,
  });
  const [{ isValidSignature, signature }, setSignature] = useState<{ isValidSignature: boolean; signature: string }>({
    isValidSignature: false,
    signature: "",
  });
  const [cryptoOptions] = useState(
    [{ text: t<string>("Crypto not detected"), value: "unknown" }].concat(settings.availableCryptos as any[])
  );
