...
function DesignKusama({ accountId }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <>
      <Button icon="pen-nib" onClick={toggleDesign} />
      {isShowing && (
        <Modal header={t("design samples")} size="large">
          <Modal.Content>
            <canvas height={HEIGHT} ref={canvasRef} style={CANVAS_STYLE} width={WIDTH} />
          </Modal.Content>
          <Modal.Actions cancelLabel={t<string>("Close")} onCancel={toggleDesign} />
        </Modal>
      )}
    </>
  );
