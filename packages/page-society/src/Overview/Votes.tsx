...
function Votes({ votes }: Props): React.ReactElement<Props> {
  ...
  const [{ allAye, allNay, allSkeptic }, setVoteSplit] = useState<VoteSplit>({
    allAye: [],
    allNay: [],
    allSkeptic: [],
  });
  ...
  return (
    <td className="expand">
      <VotesExpander label={t<string>("Skeptics")} votes={allSkeptic} />
      <VotesExpander label={t<string>("Approvals")} votes={allAye} />
      <VotesExpander label={t<string>("Rejections")} votes={allNay} />
    </td>
  );
}
