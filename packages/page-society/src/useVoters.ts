...
async function getVoters(api: ApiPromise, candidates: DeriveSocietyCandidate[]): Promise<Voters> {
  const entries = await Promise.all(candidates.map(({ accountId }) => api.query.society.votes.entries(accountId)));
...
export default function useVoters(): Voters {
  ...
  useEffect((): void => {
    voteTrigger &&
      candidates &&
      (candidates.length ? getVoters(api, candidates).then(setState).catch(console.error) : setState(EMPTY));
