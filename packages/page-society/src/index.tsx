...
function getMapMembers(
  ...
  skeptics: string[],
  voters: string[],
  { defender, founder, hasDefender, head }: DeriveSociety,
  warnStrikes: BN
): [MapMember[], BN] {
  ...
  return [
    mapMembers,
    mapMembers.reduce(
      (total, { payouts }) => payouts.reduce((total, [, balance]) => total.iadd(balance), total),
      new BN(0)
    ),
  ];
}
    ...
    <main className={className}>
      ...
      <Switch>
        <Route path={`${basePath}/candidates`}>
          <Candidates allMembers={allMembers} candidates={candidates} isMember={isMember} ownMembers={ownMembers} />
        </Route>
