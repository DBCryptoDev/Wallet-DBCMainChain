...
const optExtractAccounts = {
  transform: (keys: StorageKey<[AccountId]>[]): AccountId[] =>
    keys.map(({ args: [accountId] }) => accountId).filter((a) => !!a),
};

function Suspended({ className }: Props): React.ReactElement<Props> {
  const { t } = useTranslation();
  const { api } = useApi();
  const candidates = useCall<CandidateSuspend[]>(
  ...
  return (
    <div className={className}>
      ...
      >
        {members?.map(
          (accountId): React.ReactNode => (
            <Suspension key={accountId.toString()} value={accountId} />
          )
        )}
      </Table>
      <Table
      ...
      >
        {candidates?.map(
          ({ accountId, balance, bid }): React.ReactNode => (
            <Suspension balance={balance} bid={bid} key={accountId.toString()} value={accountId} />
          )
        )}
      </Table>
    </div>
