...
function Candidates({ allMembers, candidates, className, isMember, ownMembers }: Props): React.ReactElement<Props> {
  ...
  return (
    <div className={className}>
      <Button.Group>
        <Button icon="plus" label={t<string>("Submit bid")} onClick={toggleBidOpen} />
        {isBidOpen && <BidNew onClose={toggleBidOpen} />}
      </Button.Group>
      <AllCandidates allMembers={allMembers} candidates={candidates} isMember={isMember} ownMembers={ownMembers} />
      <Bids />
    </div>
  );
}
