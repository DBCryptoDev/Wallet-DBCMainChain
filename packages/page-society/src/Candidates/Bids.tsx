...
function Bids({ className }: Props): React.ReactElement<Props> {
  ...
  return (
    <Table className={className} empty={bids && t<string>("No bids")} header={headerRef.current}>
      {bids?.map(
        (bid, index): React.ReactNode => (
          <BidRow index={index} key={bid.who.toString()} value={bid} />
        )
      )}
    </Table>
  );
