...
function Candidate({
  ...
  isMember,
  ownMembers,
  value: { accountId, kind, value },
}: Props): React.ReactElement<Props> {
  const { api } = useApi();
  const votes = useCall<VoteType[]>(
    api.query.society.votes.multi,
  ...
  return (
    <tr>
      ...
      <Votes votes={votes} />
      <td className="button">
        <CandidateVoting candidateId={accountId.toString()} isMember={isMember} ownMembers={ownMembers} />
      </td>
    </tr>
  );
}
