...
function Deploy({ codeHash, constructorIndex = 0, onClose, setConstructorIndex }: Props): React.ReactElement<Props> {
  ...
  const { contractAbi, errorText, isAbiError, isAbiSupplied, isAbiValid, onChangeAbi, onRemoveAbi } = useAbi(
    [code && code.json.abi, code && code.contractAbi],
    codeHash,
    true
  );

  const blueprint = useMemo(
    () => (isAbiValid && codeHash && contractAbi ? new BlueprintPromise(api, contractAbi, codeHash) : null),
  ...
  useEffect((): void => {
    endowment &&
      setInitTx((): SubmittableExtrinsic<"promise"> | null => {
        if (blueprint) {
          try {
            return blueprint.createContract(
              constructorIndex,
              { gasLimit: weight.weight, salt: withSalt ? salt : null, value: endowment },
              ...params
            );
          } catch (error) {
            return null;
    ...
    <Modal header={t("Deploy a contract")}>
      <Modal.Content>
        <InputAddress
          ...
          value={accountId}
        />
        <InputName isContract isError={!isNameValid} onChange={setName} value={name || ""} />
        {!isAbiSupplied && (
          <ABI
        ...
        >
          <Toggle isOverlay label={t<string>("use deployment salt")} onChange={setWithSalt} value={withSalt} />
        </Input>
        <InputMegaGas
          help={t<string>(
