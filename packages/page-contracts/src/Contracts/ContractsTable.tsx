...
function ContractsTable({ contracts: keyringContracts }: Props): React.ReactElement<Props> {
  ...
  const [{ contractIndex, messageIndex, onCallResult }, setIndexes] = useState<Indexes>({
    contractIndex: 0,
    messageIndex: 0,
  });
  ...
  useEffect((): void => {
    if (newBlock) {
      ...
      exts.length &&
        setContractLinks((links): Record<string, ContractLink[]> => {
  ...
  return (
    <>
      <Table empty={t<string>("No contracts available")} header={headerRef.current}>
        {contracts.map(
          (contract, index): React.ReactNode => (
            <Contract
              contract={contract}
              index={index}
