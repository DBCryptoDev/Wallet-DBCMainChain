...
function Contracts({ className = "" }: Props): React.ReactElement<Props> {
  ...
  return (
    <div className={className}>
      ...
      <Button.Group>
        <Button icon="plus" label={t("Upload & deploy code")} onClick={toggleUpload} />
        <Button icon="plus" label={t("Add an existing code hash")} onClick={toggleHash} />
        <Button icon="plus" label={t("Add an existing contract")} onClick={toggleAdd} />
      </Button.Group>
      <ContractsTable contracts={allContracts} updated={updated} />
      <Codes onShowDeploy={_onShowDeploy} updated={updated} />
