...
interface Props {
  ...
  onCall: (
    ...
    messaeIndex: number,
    resultCb: (messageIndex: number, result?: ContractCallOutcome) => void
  ) => void;
}
...
function Contract({ className, contract, index, links, onCall }: Props): React.ReactElement<Props> | null {
  ...
  const info = useCall<ContractInfo | null>(api.query.contracts.contractInfoOf, [contract.address], {
  ...
  return (
    <tr className={className}>
      ...
      <td className="all top">
        <Messages contract={contract} contractAbi={contract.abi} isWatching onSelect={_onCall} withMessages />
      </td>
      <td className="top">
        {links?.map(
          ({ blockHash, blockNumber }, index): React.ReactNode => (
            <a href={`#/explorer/query/${blockHash}`} key={`${index}-${blockNumber}`}>
              #{blockNumber}
            </a>
          )
        )}
      ...
      <td className="number">
        <AddressInfo address={contract.address} withBalance withBalanceToggle withExtended={false} />
      </td>
      <td className="start together">{!isUndefined(info) && (info ? info.type : t<string>("Not on-chain"))}</td>
      <td className="number together media--1100">
        {bestNumber &&
          (evictAt ? (
            <>
              <BlockToTime value={evictAt.sub(bestNumber)} />#{formatNumber(evictAt)}
            </>
          ) : (
            t<string>("None")
          ))}
