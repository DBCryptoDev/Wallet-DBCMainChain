  ...
  return (
    <Modal header={t("Add an existing contract")}>
      <Modal.Content>
        <AddressRow defaultName={name} isValid value={address || null}>
          <Input
            ...
            value={address || ""}
          />
          <ValidateAddr address={address} onChange={setIsAddressValid} />
          <InputName isContract isError={!isNameValid} onChange={setName} value={name || undefined} />
          <ABI
      ...
      <Modal.Actions onCancel={onClose}>
        <Button icon="save" isDisabled={!isValid} label={t<string>("Save")} onClick={_onAdd} />
      </Modal.Actions>
    </Modal>
  );
}
