...
function Call({
  ...
  useEffect((): void => {
    ...
    contract
      ...
      .send(accountId)
      .then(({ gasConsumed, result }) => setEstimatedWeight(result.isOk ? gasConsumed : null))
  ...
  const _onClearOutcome = useCallback(
    (outcomeIndex: number) => () => setOutcomes([...outcomes.filter((_, index) => index !== outcomeIndex)]),
    [outcomes]
  );
  ...
  return (
    ...
    >
      <Modal.Content>
        ...
        {messageIndex !== null && (
          <>
            <Dropdown
              ...
              value={messageIndex}
            />
            <Params onChange={setParams} params={message ? message.args : undefined} registry={contract.abi.registry} />
          </>
        )}
        {message.isPayable && (
          <InputBalance
        ...
        {outcomes.length > 0 && (
          <Expander className="outcomes" isOpen summary={t<string>("Call results")}>
            {outcomes.map(
      ...
      <Modal.Actions onCancel={onClose}>
        {isViaRpc ? (
          <Button icon="sign-in-alt" isDisabled={!isValid} label={t<string>("Read")} onClick={_onSubmitRpc} />
        ) : (
          <TxButton
            accountId={accountId}
            extrinsic={execTx}
