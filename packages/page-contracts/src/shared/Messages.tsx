...
function sortMessages(messages: AbiMessage[]): [AbiMessage, number][] {
  return messages
    ...
    .sort((a, b) => (a[0].isMutating === b[0].isMutating ? 0 : a[0].isMutating ? -1 : 1));
}
      ...
      {withConstructors && (
        <Expander summary={t<string>("Constructors ({{count}})", { replace: { count: constructors.length } })}>
          {sortMessages(constructors).map(([message, index]) => (
            <Message index={index} key={index} message={message} onSelect={onSelectConstructor} />
          ))}
        </Expander>
      )}
      {withMessages && (
        ...
        >
          {sortMessages(messages).map(([message, index]) => (
            <Message index={index} key={index} lastResult={lastResults[index]} message={message} onSelect={_onSelect} />
          ))}
        </Expander>
      )}
