...
function InputMegaGas({
  ...
  const estimatedMg = useMemo(
    () => (estimatedWeight ? estimatedWeight.div(BN_MILLION).iadd(BN_ONE) : null),
    [estimatedWeight]
  );
    ...
    <div className={className}>
      <InputNumber
        ...
        label={
          estimatedMg && (isCall ? !withEstimate : true)
            ? t<string>("max gas allowed (M, {{estimatedMg}} estimated)", {
                replace: { estimatedMg: estimatedMg.toString() },
              })
      ...
      >
        {(estimatedWeight || isCall) && (
          <Toggle
            isOverlay
            label={isCall ? t<string>("max read gas") : t<string>("use estimated gas")}
