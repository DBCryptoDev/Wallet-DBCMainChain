...
function Upload({ onClose }: Props): React.ReactElement {
  ...
  const [[uploadTx, error], setUploadTx] = useState<[SubmittableExtrinsic<"promise"> | null, string | null]>([
    null,
    null,
  ]);
  ...
  const code = useMemo(
    () => (isAbiValid && isWasmValid && wasm && contractAbi ? new CodePromise(api, contractAbi, wasm) : null),
    ...
    () =>
      contractAbi
        ? contractAbi.constructors.map((message, index) => ({
            ...
            key: `${index}`,
            text: <MessageSignature asConstructor message={message} />,
            value: index,
          }))
        : [],
    [contractAbi]
  ...
  useEffect((): void => {
    setConstructorIndex(() => constructOptions.find(({ info }) => info === "default")?.value || 0);
  }, [constructOptions]);
  ...
  useEffect((): void => {
    setWasm(
      contractAbi && isWasm(contractAbi.project.source.wasm) ? [contractAbi.project.source.wasm, true] : [null, false]
    );
  }, [contractAbi]);
    ...
    <Modal header={t("Upload & deploy code {{info}}", { replace: { info: `${step}/2` } })}>
      <Modal.Content>
        {step === 1 && (
          <>
            <InputAddress
              ...
              labelExtra={<Available label={t<string>("transferrable")} params={accountId} />}
            ...
            {!hasBatchDeploy && (
              <MarkError
                content={t<string>(
                  "Your environment does not support the latest instantiateWithCode contracts call, nor does it have utility.batch functionality available. This operation is not available."
                )}
              />
            )}
              ...
              <>
                {!contractAbi.project.source.wasm.length && (
                  <InputFile
                    ...
                    onChange={_onAddWasm}
                    placeholder={
                      wasm && !isWasmValid && t<string>("The code is not recognized as being in valid WASM format")
                    }
                  />
                )}
                <InputName isError={!isNameValid} onChange={setName} value={name || undefined} />
              </>
            )}
          </>
        )}
        {step === 2 && contractAbi && (
          <>
            ...
            <InputBalance
              help={t<string>(
                "The allotted endowment for the deployed contract, i.e. the amount transferred to the contract upon instantiation."
              )}
      ...
      <Modal.Actions onCancel={onClose}>
        {step === 1 ? (
          ...
          <Button icon="step-backward" label={t<string>("Prev")} onClick={prevStep} />
        )}
        <TxButton
          accountId={accountId}
          extrinsic={uploadTx}
