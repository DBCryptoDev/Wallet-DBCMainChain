...
function WrapToggle({ children, onChange, value }: WrapProps): React.ReactElement<WrapProps> {
  ...
  return (
    <div className="toggle-Wrap">
      {children}
      <Toggle isOverlay label={t<string>("include field")} onChange={onChange} value={value} />
    </div>
  );
}
...
function IdentityMain({ address, className = "", onClose }: Props): React.ReactElement<Props> {
  ...
  const [{ info, okAll, okDisplay, okEmail, okLegal, okRiot, okTwitter, okWeb }, setInfo] = useState<ValueState>({
    info: {},
    okAll: false,
  });
  ...
  return (
    <Modal className={className} header={t<string>("Register identity")}>
      <Modal.Content>
        <Input
          autoFocus
        ...
        <InputBalance
          ...
          help={t<string>(
            "Total amount of fund that will be reserved. These funds are returned when the identity is cleared"
          )}
          isDisabled
