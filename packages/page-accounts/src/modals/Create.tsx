...
function createAccount(
  ...
  try {
    const result = keyring.addUri(
      getSuri(seed, derivePath, pairType),
      password,
      { genesisHash, isHardware: false, name, tags },
      pairType === "ed25519-ledger" ? "ed25519" : pairType
    );
...
function Create({
  ...
  const [{ address, derivePath, deriveValidation, isSeedValid, pairType, seed, seedType }, setAddress] =
  ...
  const seedOpt = useRef(
    (isDevelopment ? [{ text: t<string>("Development"), value: "dev" }] : []).concat(
  ...
  const _onChangePairType = useCallback(
    (newPairType: PairType) =>
      setAddress(updateAddress(seed, isEthereum ? ETH_DEFAULT_PATH : "", seedType, newPairType)),
  ...
  const _onChangeName = useCallback((name: string) => setName({ isNameValid: !!name.trim(), name }), []);
  ...
  return (
    ...
    >
      <Modal.Content>
            ...
            >
              ...
              >
                <CopyButton
                  ...
                  value={seed}
                />
                <Dropdown defaultValue={seedType} isButton onChange={_selectSeedType} options={seedOpt.current} />
              </TextArea>
            </Modal.Columns>
            <Expander className="accounts--Creator-advanced" isPadded summary={t<string>("Advanced creation options")}>
              <Modal.Columns
                hint={t<string>(
              ...
              {pairType === "ed25519-ledger" ? (
                <CreateSuriLedger onChange={_onChangePath} seedType={seedType} />
              ) : (
                <Modal.Columns
                  hint={
        ...
        {step === 2 && (
          <>
            ...
            <PasswordInput onChange={_onPasswordChange} onEnter={_onCommit} />
            <Modal.Columns>
              <ExternalWarning />
            </Modal.Columns>
          </>
      ...
      <Modal.Actions onCancel={onClose}>
        {step === 1 && (
          <Button icon="step-forward" isDisabled={!isFirstStepValid} label={t<string>("Next")} onClick={nextStep} />
        )}
        {step === 2 && (
          <>
            <Button icon="step-backward" label={t<string>("Prev")} onClick={prevStep} />
            <Button icon="step-forward" isDisabled={!isSecondStepValid} label={t<string>("Next")} onClick={nextStep} />
          </>
        )}
        {step === 3 && (
          <>
            <Button icon="step-backward" label={t<string>("Prev")} onClick={prevStep} />
            <Button icon="plus" isBusy={isBusy} label={t<string>("Save")} onClick={_onCommit} />
          </>
        )}
      </Modal.Actions>
    </Modal>
