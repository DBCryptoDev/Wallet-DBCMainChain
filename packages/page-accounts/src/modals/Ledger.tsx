...
function LedgerModal({ className, onClose }: Props): React.ReactElement<Props> {
  ...
  return (
    <Modal className={className} header={t<string>("Add account via Ledger")} size="large">
      <Modal.Content>
        ...
        >
          <Dropdown
            ...
            value={addIndex}
          />
          {error && <MarkError content={error.message} />}
        </Modal.Columns>
      </Modal.Content>
      <Modal.Actions onCancel={onClose}>
        <Button icon="plus" isBusy={isBusy} isDisabled={!isNameValid} label={t<string>("Save")} onClick={_onSave} />
      </Modal.Actions>
    </Modal>
  );
}
