...
function createExtrinsic(
  api: ApiPromise,
  batchPrevious: SubmittableExtrinsic<"promise">[],
  batchAdded: SubmittableExtrinsic<"promise">[]
): SubmittableExtrinsic<"promise"> | null {
  if (batchPrevious.length + batchAdded.length === 1) {
    return batchPrevious.length ? batchPrevious[0] : batchAdded[0];
  }
...
}: PrevProxyProps): React.ReactElement<PrevProxyProps> {
  ...
  return (
    <div className="proxy-container">
      <div className="input-column">
        <InputAddress defaultValue={accountId} isDisabled label={t<string>("proxy account")} />
        <Dropdown help={"Type of proxy"} isDisabled label={"type"} options={typeOpts} value={type.toNumber()} />
      </div>
...
function NewProxy({
  ...
  typeOpts,
  value: [accountId, type],
}: NewProxyProps): React.ReactElement<NewProxyProps> {
  const { t } = useTranslation();
  ...
  const _onChangeType = useCallback((value?: number) => onChangeType(index, value), [index, onChangeType]);

  const _onRemove = useCallback(() => onRemove(index), [index, onRemove]);

  return (
    <div className="proxy-container" key={`addedProxy-${index}`}>
      <div className="input-column">
        <InputAddress label={t<string>("proxy account")} onChange={_onChangeAccount} type="account" value={accountId} />
...
function ProxyOverview({
  ...
  const typeOpts = useRef(api.createType("ProxyType").defKeys.map((text, value) => ({ text, value })));
  ...
  useEffect((): void => {
    (batchPrevious.length || batchAdded.length) && setExtrinsic(() => createExtrinsic(api, batchPrevious, batchAdded));
  ...
  const _delProxy = useCallback((index: number) => setAdded((added) => added.filter((_, i) => i !== index)), []);
  ...
  return (
    <Modal className={className} header={t<string>("Proxy overview")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={t<string>("Any account set as proxy will be able to perform actions in place of the proxied account")}
        >
          <InputAddress isDisabled={true} label={t<string>("proxied account")} type="account" value={proxiedAccount} />
        </Modal.Columns>
        <Modal.Columns
          hint={t<string>(
            "If you add several proxy accounts for the same proxy type (e.g 2 accounts set as proxy for Governance), then any of those 2 accounts will be able to perfom governance actions on behalf of the proxied account"
          )}
        >
          ...
          <Button.Group>
            <Button icon="plus" label={t<string>("Add proxy")} onClick={_addProxy} />
          </Button.Group>
        </Modal.Columns>
        <Modal.Columns>
          <BatchWarning />
