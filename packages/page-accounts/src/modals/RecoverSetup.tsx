...
function RecoverSetup({ address, className = "", onClose }: Props): React.ReactElement {
  ...
  return (
    <Modal className={className} header={t<string>("Setup account as recoverable")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={t<string>("The recoverable account is protected against the loss of seed/access by a social process.")}
        >
          <InputAddress isDisabled label={t<string>("the account to make recoverable")} value={address} />
        </Modal.Columns>
        <Modal.Columns
          hint={
            <>
        ...
        >
          <InputAddressMulti
            ...
            help={t<string>(
              "The addresses that are able to help in recovery. You can select up to {{maxHelpers}} trusted helpers.",
              { replace: { maxHelpers: MAX_HELPERS } }
            )}
