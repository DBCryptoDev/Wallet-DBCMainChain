...
function Multisig({ className = "", onClose, onStatusChange }: Props): React.ReactElement<Props> {
  ...
  const _onChangeName = useCallback((name: string) => setName({ isNameValid: name.trim().length >= 3, name }), []);

  const _onChangeThreshold = useCallback(
    (threshold: BN | undefined) =>
      threshold &&
      setThreshold({ isThresholdValid: threshold.gte(BN_TWO) && threshold.lten(signatories.length), threshold }),
    [signatories]
  );
  ...
  return (
    <Modal className={className} header={t<string>("Add multisig")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={
            <>
        ...
        >
          <InputAddressMulti
            ...
            help={t<string>(
              "The addresses that are able to approve multisig transactions. You can select up to {{maxHelpers}} trusted addresses.",
              { replace: { maxHelpers: MAX_SIGNATORIES } }
            )}
      ...
      <Modal.Actions onCancel={onClose}>
        <Button icon="plus" isDisabled={!isValid} label={t<string>("Create")} onClick={_createMultisig} />
      </Modal.Actions>
    </Modal>
  );
}
