...
function Derive({ className = "", from, onClose }: Props): React.ReactElement {
  ...
  const [{ isLocked, lockedError }, setIsLocked] = useState<LockState>({
  ...
  const _onChangeRootPass = useCallback((rootPass: string): void => {
  ...
  const _onCommit = useCallback((): void => {
    ...
    setTimeout((): void => {
      const status = createAccount(
        ...
        name,
        password,
        t<string>("created account"),
        isDevelopment ? undefined : api.genesisHash.toString()
      );
  ...
  return (
    <Modal className={className} header={t<string>("Derive account from pair")}>
      {address && isConfirmationOpen ? (
        <CreateConfirmation address={address} derivePath={suri} isBusy={isBusy} name={name} pairType={source.type} />
      ) : (
        <Modal.Content>
          {isLocked && (
            <>
          ...
          {!isLocked && (
            <AddressRow defaultName={name} noDefaultNameOpacity value={deriveError ? "" : address}>
              ...
              />
              {deriveError && <MarkError content={deriveError} />}
              <Input
                className="full"
                help={t<string>(
      ...
      <Modal.Actions onCancel={onClose}>
        {isLocked ? (
          ...
          <>
            <Button icon="step-backward" label={t<string>("Prev")} onClick={toggleConfirmation} />
            <Button icon="plus" isBusy={isBusy} label={t<string>("Save")} onClick={_onCommit} />
          </>
        ) : (
          <Button icon="step-forward" isDisabled={!isValid} label={t<string>("Next")} onClick={toggleConfirmation} />
        )}
      </Modal.Actions>
    </Modal>
  );
