...
function ProxyAdd({ className = "", onClose, onStatusChange }: Props): React.ReactElement<Props> {
  ...
  const _createProxied = useCallback((): void => {
    if (stashAddress) {
  ...
  return (
    <Modal className={className} header={t<string>("Add proxied account")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={t<string>("The address that has previously setup a proxy to one of the accounts that you control.")}
        >
          <InputAddressSimple
            autoFocus
      ...
      <Modal.Actions onCancel={onClose}>
        <Button icon="plus" isDisabled={!isValid} label={t<string>("Add")} onClick={_createProxied} />
      </Modal.Actions>
    </Modal>
  );
}
