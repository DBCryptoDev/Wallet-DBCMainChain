...
function ChangePass({ address, className = "", onClose }: Props): React.ReactElement<Props> {
  ...
  const _onChangeNew1 = useCallback(
    (password: string) => setNewPass1({ isValid: keyring.isPassValid(password), password }),
    []
  );
  ...
  const _doChange = useCallback((): void => {
  ...
  return (
    <Modal className={`${className} app--accounts-Modal`} header={t<string>("Change account password")} size="large">
      <Modal.Content>
