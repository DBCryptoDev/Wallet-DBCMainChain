...
function Import({ className = "", onClose, onStatusChange }: Props): React.ReactElement<Props> {
  ...
  const differentGenesis = useMemo(
    () => pair?.meta.genesisHash && pair.meta.genesisHash !== apiGenesisHash,
    [apiGenesisHash, pair]
  );

  const _onChangeFile = useCallback((file: Uint8Array) => setPair(parseFile(file, apiGenesisHash)), [apiGenesisHash]);
  ...
  return (
    <Modal className={className} header={t<string>("Add via backup file")} size="large">
      <Modal.Content>
        <Modal.Columns>
          <AddressRow
