...
function Delegate({
  ...
  const isDirty =
    amount?.toString() !== previousAmount?.toString() ||
  ...
  return (
    <Modal
      ...
      header={previousDelegatedAccount ? t<string>("democracy vote delegation") : t<string>("delegate democracy vote")}
      size="large"
    >
      <Modal.Content>
        <Modal.Columns
        ...
        >
          <InputBalance
            autoFocus
            help={t<string>(
              "Amount to delegate for any democracy vote. This is adjusted using the available funds on the account."
            )}
            isError={!!amountError?.error}
            isZeroable={false}
            label={t<string>("delegating amount")}
            labelExtra={
              <BalanceFree label={<span className="label">{t<string>("balance")}</span>} params={delegatingAccount} />
            }
            maxValue={maxBalance}
            onChange={setAmount}
            value={amount}
          />
          <ValidateAmount amount={amount} delegatingAccount={delegatingAccount} onError={setAmountError} />
          <ConvictionDropdown
      ...
      <Modal.Actions onCancel={onClose}>
        ...
        <TxButton
          ...
          label={previousDelegatedAccount ? t<string>("Save delegation") : t<string>("Delegate")}
          onStart={onClose}
