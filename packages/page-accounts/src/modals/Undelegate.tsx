...
function Undelegate({ accountDelegating, onClose }: Props): React.ReactElement<Props> {
  ...
  return (
    <Modal className="staking--Undelegate" header={t<string>("Undelegate")} size="large">
      <Modal.Content>
        <Modal.Columns hint={t<string>("You will remove any delegation made by this acccount")}>
          <InputAddress defaultValue={accountDelegating} isDisabled label={t<string>("delegating account")} />
        </Modal.Columns>
      </Modal.Content>
