...
function MultisigApprove({ className = "", onClose, ongoing, threshold, who }: Props): React.ReactElement<Props> {
  ...
  const hashes = useMemo<Option[]>(() => ongoing.map(([h]) => ({ text: h.toHex(), value: h.toHex() })), [ongoing]);
  ...
  useEffect((): void => {
    ...
    setWhoFilter(
      who
        ...
        .filter(
          (w) =>
            allAccounts.some((a) => a === w) &&
            multisig &&
            (type === "nay" ? multisig.approvals[0].eq(w) : hasThreshold || !multisig.approvals.some((a) => a.eq(w)))
        )
    );
  ...
  const _setCallData = useCallback(
    (callHex: string): void => {
      try {
        ...
        setCallData(callData.hash.eq(hash) ? callData : null);
      } catch (error) {
        setCallData(null);
      }
    },
  ...
  return (
    <Modal className={className} header={t<string>("Pending call hashes")} size="large">
      <Modal.Content>
