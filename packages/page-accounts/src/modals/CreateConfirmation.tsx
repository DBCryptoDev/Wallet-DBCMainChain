...
function CreateConfirmation({ address, derivePath, name, pairType, seed }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <Modal.Content>
      ...
      >
        {address && name && <AddressRow defaultName={name} isInline noDefaultNameOpacity value={address} />}
        {shortSeed && <Static label={t<string>("partial seed")} value={shortSeed} />}
        <Static label={t<string>("keypair type")} value={pairType} />
        <Static label={t<string>("derivation path")} value={derivePath || t<string>("<none provided>")} />
      </Modal.Columns>
    </Modal.Content>
  );
}
