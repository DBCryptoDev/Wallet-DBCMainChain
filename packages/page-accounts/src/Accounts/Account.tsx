...
function createClearDemocracyTx(
  api: ApiPromise,
  address: string,
  unlockableIds: BN[]
  ...
  return api.tx.utility
    ? api.tx.utility.batch(
        unlockableIds.map((id) => api.tx.democracy.removeVote(id)).concat(api.tx.democracy.unlock(address))
      )
    : null;
}
...
function Account({
  ...
  const recoveryInfo = useCall<RecoveryConfig | null>(
    api.api.query.recovery?.recoverable,
    [address],
    transformRecovery
  );
  ...
  const {
    flags: { isDevelopment, isEditable, isExternal, isHardware, isInjected, isMultisig, isProxied },
    genesisHash,
    identity,
    name: accName,
    onSetGenesisHash,
  ...
  useEffect((): void => {
    if (balancesAll) {
      ...
      api.api.tx.vesting?.vest &&
        setVestingTx(() => (balancesAll.vestingLocked.isZero() ? null : api.api.tx.vesting.vest()));
    }
  ...
  useEffect((): void => {
    bestNumber &&
      democracyLocks &&
      setUnlockableIds((prev): DemocracyUnlockable => {
      ...
      [
        ...
        createMenuGroup("deriveGroup", [
          !(isExternal || isHardware || isInjected || isMultisig) && (
            <Menu.Item key="deriveAccount" onClick={toggleDerive}>
              {t("Derive account via derivation path")}
            </Menu.Item>
          ),
          isHardware && (
            <Menu.Item key="showHwAddress" onClick={_showOnHardware}>
        ...
        createMenuGroup("backupGroup", [
          !(isExternal || isHardware || isInjected || isMultisig || isDevelopment) && (
            <Menu.Item key="backupJson" onClick={toggleBackup}>
              {t("Create a backup file for this account")}
            </Menu.Item>
          ),
          !(isExternal || isHardware || isInjected || isMultisig || isDevelopment) && (
            <Menu.Item key="changePassword" onClick={togglePassword}>
              {t("Change this account's password")}
            </Menu.Item>
          ),
          !(isInjected || isDevelopment) && (
            <Menu.Item key="forgetAccount" onClick={toggleForget}>
              {t("Forget this account")}
            </Menu.Item>
          ),
        ]),
        isFunction(api.api.tx.recovery?.createRecovery) &&
          createMenuGroup("reoveryGroup", [
            !recoveryInfo && (
              <Menu.Item key="makeRecoverable" onClick={toggleRecoverSetup}>
                {t("Make recoverable")}
              </Menu.Item>
            ),
            <Menu.Item key="initRecovery" onClick={toggleRecoverAccount}>
              {t("Initiate recovery for another")}
            </Menu.Item>,
          ]),
        isFunction(api.api.tx.multisig?.asMulti) &&
          isMultisig &&
          createMenuGroup("multisigGroup", [
            <Menu.Item disabled={!multiInfos || !multiInfos.length} key="multisigApprovals" onClick={toggleMultisig}>
              {t("Multisig approvals")}
            </Menu.Item>,
          ]),
        isFunction(api.api.query.democracy?.votingOf) &&
          delegation?.accountDelegated &&
          createMenuGroup("undelegateGroup", [
            <Menu.Item key="changeDelegate" onClick={toggleDelegate}>
              {t("Change democracy delegation")}
            </Menu.Item>,
            <Menu.Item key="undelegate" onClick={toggleUndelegate}>
              {t("Undelegate")}
            </Menu.Item>,
          ]),
        isFunction(api.api.query.democracy?.votingOf) &&
          ...
          createMenuGroup("delegateGroup", [
            <Menu.Item key="delegate" onClick={toggleDelegate}>
              {t("Delegate democracy votes")}
            </Menu.Item>,
          ]),
        isFunction(api.api.query.proxy?.proxies) &&
          createMenuGroup("proxyGroup", [
            <Menu.Item key="proxy-overview" onClick={toggleProxyOverview}>
              {proxy?.[0].length ? t("Manage proxies") : t("Add proxy")}
            </Menu.Item>,
          ]),
        isEditable &&
          !api.isDevelopment &&
          createMenuGroup("genesisGroup", [
            <ChainLock
  ...
  return (
    <tr className={className}>
      <td className="favorite">
        <Icon color={isFavorite ? "orange" : "gray"} icon="star" onClick={_onFavorite} />
      </td>
      <td className="together">
                ...
                <div>
                  {recoveryInfo.friends.map(
                    (friend, index): React.ReactNode => (
                      <IdentityIcon key={index} value={friend} />
                    )
                  )}
                </div>
                <table>
        ...
        {multiInfos && multiInfos.length !== 0 && (
          <Badge color="red" hover={t<string>("Multisig approvals pending")} info={multiInfos.length} />
        )}
        {isProxied && !proxyInfo.hasOwned && (
          <Badge color="red" hover={t<string>("Proxied account has no owned proxies")} info="0" />
        )}
        {delegation?.accountDelegated && (
          <Badge
            color="blue"
      ...
      <td className="address">
        ...
        {isBackupOpen && <Backup address={address} key="modal-backup-account" onClose={toggleBackup} />}
        {isDelegateOpen && (
          <DelegateModal
            key="modal-delegate"
        ...
        {isDeriveOpen && <Derive from={address} key="modal-derive-account" onClose={toggleDerive} />}
        {isForgetOpen && (
          <Forget address={address} key="modal-forget-account" onClose={toggleForget} onForget={_onForget} />
        )}
        {isIdentityMainOpen && (
          <IdentityMain address={address} key="modal-identity-main" onClose={toggleIdentityMain} />
        )}
        {isIdentitySubOpen && <IdentitySub address={address} key="modal-identity-sub" onClose={toggleIdentitySub} />}
        {isPasswordOpen && <ChangePass address={address} key="modal-change-pass" onClose={togglePassword} />}
        {isTransferOpen && <Transfer key="modal-transfer" onClose={toggleTransfer} senderId={address} />}
        {isProxyOverviewOpen && (
          <ProxyOverview
        ...
        {isRecoverAccountOpen && (
          <RecoverAccount address={address} key="recover-account" onClose={toggleRecoverAccount} />
        )}
        {isRecoverSetupOpen && <RecoverSetup address={address} key="recover-setup" onClose={toggleRecoverSetup} />}
        {isUndelegateOpen && (
          <UndelegateModal accountDelegating={address} key="modal-delegate" onClose={toggleUndelegate} />
        )}
      </td>
      <td className="address media--1400">
        {(meta.parentAddress as string) && <AddressMini value={meta.parentAddress} />}
      </td>
      <td className="number">
      ...
      <td className="button">
        ...
        <Popup
          ...
          trigger={<Button icon="ellipsis-v" isDisabled={!menuItems.length} onClick={toggleSettings} />}
        >
          <Menu onClick={toggleSettings} text vertical>
            {menuItems}
          </Menu>
        </Popup>
      </td>
      <td className="links media--1400">
        <LinkExternal className="ui--AddressCard-exporer-link" data={address} isLogo type="address" />
      </td>
    </tr>
  );
}
