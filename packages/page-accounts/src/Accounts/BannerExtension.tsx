...
function BannerExtension(): React.ReactElement | null {
  ...
  if (isWeb3Injected) {
    if (hasInjectedAccounts) {
      ...
      return (
        <Banner type="warning">
          <p>
            {t<string>(
              "You have {{upgradableCount}} extensions that need to be updated with the latest chain properties in order to display the correct information for the chain you are connected to. This update includes chain metadata and chain properties.",
              { replace: { upgradableCount } }
            )}
          </p>
          <p>
            <Trans key="extensionUpgrade">
              Visit your <a href="#/settings/metadata">settings page</a> to apply the updates to the injected
              extensions.
            </Trans>
          </p>
        </Banner>
    ...
    return (
      <Banner type="warning">
        <p>
          {t<string>("One or more extensions are detected in your browser, however no accounts has been injected.")}
        </p>
        <p>
          {t<string>(
  ...
  return (
    <Banner type="warning">
      ...
      <ul>
        {availableExtensions[browserName].map(
          ({ desc, link, name }): React.ReactNode => (
            <li key={name}>
