...
function Overview({ className = "", onStatusChange }: Props): React.ReactElement<Props> {
    ...
    transform: (result: [([AccountId, ProxyType] | ProxyDefinition)[], BN][]): [ProxyDefinition[], BN][] =>
      api.tx.proxy.addProxy.meta.args.length === 3
        ...
        : (result as [[AccountId, ProxyType][], BN][]).map(([arr, bn]): [ProxyDefinition[], BN] => [
            arr.map(
              ([delegate, proxyType]): ProxyDefinition => api.createType("ProxyDefinition", { delegate, proxyType })
            ),
            bn,
          ]),
  });
  ...
  const footer = useMemo(
    () => (
      <tr>
        ...
        <td className="number">{balanceTotal && <FormatBalance value={balanceTotal} />}</td>
        <td />
        <td className="media--1400" />
      </tr>
    ),
  ...
  return (
    <div className={className}>
      ...
      {isImportOpen && <ImportModal onClose={toggleImport} onStatusChange={onStatusChange} />}
      {isLedgerOpen && <Ledger onClose={toggleLedger} />}
      {isMultisigOpen && <Multisig onClose={toggleMultisig} onStatusChange={onStatusChange} />}
      {isProxyOpen && <Proxy onClose={toggleProxy} onStatusChange={onStatusChange} />}
      {isQrOpen && <Qr onClose={toggleQr} onStatusChange={onStatusChange} />}
      <Button.Group>
        <Button icon="plus" isDisabled={isIpfs} label={t<string>("Add account")} onClick={toggleCreate} />
        <Button icon="sync" isDisabled={isIpfs} label={t<string>("Restore JSON")} onClick={toggleImport} />
        <Button icon="qrcode" label={t<string>("Add via Qr")} onClick={toggleQr} />
        {isLedgerEnabled && (
          <>
            <Button icon="project-diagram" label={t<string>("Add via Ledger")} onClick={toggleLedger} />
          </>
        )}
        <Button
          icon="plus"
      ...
      >
        {!isLoading &&
          sortedAccountsWithDelegation?.map(
            ({ account, delegation, isFavorite }, index): React.ReactNode => (
              <Account
                account={account}
