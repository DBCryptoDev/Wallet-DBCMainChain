...
function Address({
  ...
  const _setTags = useCallback((tags: string[]): void => setTags(tags.sort()), []);
  ...
  const _onFavorite = useCallback((): void => toggleFavorite(address), [address, toggleFavorite]);

  const _toggleForget = useCallback((): void => setIsForgetOpen(!isForgetOpen), [isForgetOpen]);

  const _toggleSettingPopup = useCallback((): void => setIsSettingPopupOpen(!isSettingPopupOpen), [isSettingPopupOpen]);

  const _toggleTransfer = useCallback((): void => setIsTransferOpen(!isTransferOpen), [isTransferOpen]);
  ...
  return (
    <tr className={className}>
      ...
      <td className="address">
        ...
        {address && current && (
          <>
            ...
            {isTransferOpen && <Transfer key="modal-transfer" onClose={_toggleTransfer} recipientId={address} />}
          </>
        )}
      </td>
      <td className="all">
      ...
      <td className="number">
        <AddressInfo address={address} withBalance={WITH_BALANCE} withBalanceToggle withExtended={false} />
      </td>
      <td className="button">
        {isFunction(api.api.tx.balances?.transfer) && (
          <Button icon="paper-plane" key="send" label={t<string>("send")} onClick={_toggleTransfer} />
        )}
        <Popup
          className={`theme--${theme}`}
          isOpen={isSettingPopupOpen}
          onClose={_toggleSettingPopup}
          trigger={<Button icon="ellipsis-v" onClick={_toggleSettingPopup} />}
        >
          <Menu onClick={_toggleSettingPopup} text vertical>
            <Menu.Item disabled={!isEditable} onClick={_toggleForget}>
