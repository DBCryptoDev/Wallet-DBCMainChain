...
function Create({ onClose, onStatusChange }: Props): React.ReactElement<Props> {
  ...
  const [{ address, addressInput, isAddressExisting, isAddressValid }, setAddress] = useState<AddrState>({
    address: "",
    addressInput: "",
  ...
  const _onCommit = useCallback((): void => {
    ...
    try {
      ...
      status.message = isAddressExisting ? t<string>("address edited") : t<string>("address created");
    ...
    <Modal header={t<string>("Add an address")}>
      <Modal.Content>
        <AddressRow
          ...
          value={isAddressValid ? info?.accountId?.toString() : undefined}
        >
          <Input
            autoFocus
            className="full"
      ...
      <Modal.Actions onCancel={onClose}>
        <Button icon="save" isDisabled={!isValid} label={t<string>("Save")} onClick={_onCommit} />
      </Modal.Actions>
    </Modal>
  );
}
