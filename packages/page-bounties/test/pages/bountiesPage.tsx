...
export class BountiesPage {
  ...
  constructor(api: ApiPromise) {
    ({
      ...
      aBountyIndex: this.aBountyIndex,
      aBountyStatus: this.aBountyStatus,
      bountyStatusWith: this.bountyStatusWith,
      bountyWith: this.bountyWith,
    } = new BountyFactory(api));
  }
  ...
  renderMany(bountyApi: Partial<BountyApi> = {}, { balance = 1 } = {}): RenderedBountiesPage {
    const { findAllByTestId, findByRole, findByTestId, findByText, getAllByRole, queryAllByText } = this.renderBounties(
      bountyApi,
      { balance }
    );
