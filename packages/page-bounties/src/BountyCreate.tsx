...
function BountyCreate() {
  ...
  const { bountyDepositBase, bountyValueMinimum, dataDepositPerByte, maximumReasonLength, proposeBounty } =
    useBounties();
  ...
  return (
    <>
      <Button icon="plus" isDisabled={false} label={t<string>("Add Bounty")} onClick={toggleIsOpen} />
      {isOpen && (
        <Modal className="ui--AddBountyModal" header={t<string>("Add Bounty")}>
          <Modal.Content>
            <Modal.Columns hint={t<string>("Description of the Bounty (to be stored on-chain)")}>
              <Input
                ...
                value={title}
              />
              {!isTitleValid && title !== TITLE_DEFAULT_VALUE && <MarkError content={t<string>("Title too long")} />}
            </Modal.Columns>
            <Modal.Columns
              hint={t<string>(
            ...
            <Modal.Columns hint={t<string>("Proposer bond depends on bounty title length.")}>
              <InputBalance
                ...
                help={t<string>(
                  "This amount will be reserved from origin account and returned on approval or slashed upon rejection."
                )}
                isDisabled
            ...
            <Modal.Columns
              hint={t<string>("This account will propose the bounty. Bond amount will be reserved on its balance.")}
            >
              <InputAddress
                ...
                withLabel
              />
              {!hasFunds && <MarkError content={t<string>("Account does not have enough funds.")} />}
            </Modal.Columns>
          </Modal.Content>
