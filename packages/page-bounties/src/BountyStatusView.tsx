...
function BountyStatusView({ bountyStatus, className = "" }: Props): React.ReactElement<Props> {
  ...
  const statusHelpMessages = useRef<HelpMessages>({
    ...
    PendingPayout: t(
      "This bounty was completed and the beneficiary was rewarded by the curator. Claiming the payout will be possible after the delay period is over."
    ),
    Proposed: t("After a bounty was proposed the council decides whether to fund it or not."),
  });

  return (
    <div className={className} data-testid={"bountyStatus"}>
      {insertSpaceBeforeCapitalLetter(bountyStatus)}
      <LabelHelp help={statusHelpMessages.current[bountyStatus]} />
    </div>
  );
}
