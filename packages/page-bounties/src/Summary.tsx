...
function Summary({ activeBounties, className = "" }: Props): React.ReactElement<Props> {
  ...
  return (
    <SummaryBox className={`ui--BountySummary ${className}`}>
      <section>
        <CardSummary label={t<string>("active")}>{activeBounties}</CardSummary>
        {activeBounties !== undefined && (
          <CardSummary label={t<string>("past")}>{bountyIndex?.subn(activeBounties).toString()}</CardSummary>
        )}
      </section>
      <section>
        <CardSummary label={t<string>("active total")}>
          <FormatBalance value={totalValue} withSi />
        </CardSummary>
      </section>
      <section>
