...
function BountyActionMessage({ bestNumber, blocksUntilUpdate, status }: Props): JSX.Element {
  ...
  const blocksPercentageLeftToShowWarning = bountyUpdatePeriod
    ?.muln(BLOCKS_PERCENTAGE_LEFT_TO_SHOW_WARNING)
    .div(BN_HUNDRED);
  const blocksToShowWarning = blocksPercentageLeftToShowWarning ?? BLOCKS_LEFT_TO_SHOW_WARNING;

  return (
    <div>
      {blocksUntilUpdate?.lte(BN_ZERO) && <BountyInfo description={t<string>("Update overdue")} type="warning" />}
      {blocksUntilUpdate?.lt(blocksToShowWarning) && blocksUntilUpdate?.gt(BN_ZERO) && (
        <BountyInfo description={t<string>("Close deadline")} type="warning" />
      )}
      {status.isApproved && <BountyInfo description={t<string>("Waiting for Bounty Funding")} type="info" />}
      {status.isCuratorProposed && (
        <BountyInfo description={t<string>("Waiting for Curator's acceptance")} type="info" />
      )}
      {blocksUntilPayout?.lt(BN_ZERO) && (
        <BountyInfo description={t<string>("Waiting for implementer to claim")} type="info" />
      )}
    </div>
  );
}
