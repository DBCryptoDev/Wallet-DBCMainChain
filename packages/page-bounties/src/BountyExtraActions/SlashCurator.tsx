...
function SlashCurator({ action, curatorId, description, index, toggleOpen }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <Modal header={`${title} - "${truncateTitle(description, 30)}"`} size="large">
      <Modal.Content>
      ...
      <Modal.Actions onCancel={toggleOpen}>
        <TxButton accountId={accountId} icon="check" label="Approve" onStart={toggleOpen} params={params} tx={tx} />
      </Modal.Actions>
    </Modal>
  );
}
