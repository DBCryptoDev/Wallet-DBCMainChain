...
function Index({
  ...
  const existingCloseBountyProposal = useMemo(
    () => proposals?.find(({ proposal }) => proposal.method === "closeBounty"),
    [proposals]
  );
  const existingUnassignCuratorProposal = useMemo(
    () => proposals?.find(({ proposal }) => proposal.method === "unassignCurator"),
    [proposals]
  );

  const showCloseBounty =
  ...
  const showSlashCurator =
    (status.isCuratorProposed || status.isActive || status.isPendingPayout) &&
    !existingUnassignCuratorProposal &&
    availableSlashActions.length !== 0;
  ...
  return !hasNoItems ? (
    <div className={className}>
      {isCloseBountyOpen && <CloseBounty description={description} index={index} toggleOpen={toggleCloseBounty} />}
      ...
      {isGiveUpCuratorOpen && curator && (
        <GiveUp curatorId={curator} description={description} index={index} toggleOpen={toggleGiveUpCurator} />
      )}
      ...
      <Popup
        ...
        trigger={<Button dataTestId="extra-actions" icon="ellipsis-v" onClick={togglePopupOpen} />}
      >
        <Menu className="settings-menu" onClick={togglePopupOpen} text vertical>
          {showCloseBounty && (
            <Menu.Item key="closeBounty" onClick={toggleCloseBounty}>
              {t<string>("Close")}
            </Menu.Item>
          )}
          {showRejectCurator && (
            <Menu.Item key="rejectCurator" onClick={toggleRejectCurator}>
              {t<string>("Reject curator")}
            </Menu.Item>
          )}
          {showExtendExpiry && (
            <Menu.Item key="extendExpiry" onClick={toggleExtendExpiry}>
              {t<string>("Extend expiry")}
            </Menu.Item>
          )}
          {showGiveUpCurator && (
            <Menu.Item key="giveUpCurator" onClick={toggleGiveUpCurator}>
              {t<string>("Give up")}
            </Menu.Item>
          )}
          {showSlashCurator &&
            availableSlashActions.map((actionName) => (
              <Menu.Item key={actionName} onClick={() => slashCuratorClicked(actionName)}>
