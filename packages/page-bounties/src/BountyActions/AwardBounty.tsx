...
function AwardBounty({ curatorId, description, index }: Props): React.ReactElement<Props> | null {
  ...
  return isCurator ? (
    <>
      <Button icon="award" isDisabled={false} label={t<string>("Reward implementer")} onClick={toggleOpen} />
      {isOpen && (
        <Modal header={`${t<string>("award bounty")} - "${truncateTitle(description, 30)}"`} size="large">
          <Modal.Content>
            <Modal.Columns
              hint={t<string>(
