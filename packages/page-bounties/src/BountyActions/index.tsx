...
export function BountyActions({ bestNumber, description, fee, index, proposals, status, value }: Props): JSX.Element {
  ...
  return (
    <>
      {status.isProposed && <BountyInitiateVoting description={description} index={index} proposals={proposals} />}
      {status.isFunded && (
        <ProposeCuratorAction description={description} index={index} proposals={proposals} value={value} />
      )}
      {status.isCuratorProposed && curator && fee && (
        <BountyAcceptCurator curatorId={curator} description={description} fee={fee} index={index} />
      )}
      {status.isPendingPayout && beneficiary && blocksUntilPayout && (
        <BountyClaimAction beneficiaryId={beneficiary} index={index} payoutDue={blocksUntilPayout} />
      )}
      {status.isActive && curator && <AwardBounty curatorId={curator} description={description} index={index} />}
    </>
  );
}
