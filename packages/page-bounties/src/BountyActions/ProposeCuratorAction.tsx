...
function ProposeCuratorAction({ description, index, proposals, value }: Props): React.ReactElement<Props> | null {
  ...
  const isVotingInitiated = useMemo(
    () => proposals?.filter(({ proposal }) => BOUNTY_METHODS.includes(proposal.method)).length !== 0,
    [proposals]
  );
  ...
  return isMember && !isVotingInitiated ? (
    <>
      <Button icon="step-forward" isDisabled={false} label={t<string>("Propose curator")} onClick={toggleOpen} />
      {isOpen && (
        <Modal
        ...
        >
          <Modal.Content>
            ...
            <Modal.Columns
              hint={t<string>("Part of the bounty value that will go to the Curator as a reward for their work")}
            >
              <InputBalance
                ...
                value={fee}
              />
              {!isFeeValid && <MarkError content={t<string>("Curator's fee can't be higher than bounty value.")} />}
            </Modal.Columns>
          </Modal.Content>
