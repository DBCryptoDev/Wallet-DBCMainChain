...
function BountyInitiateVoting({ description, index, proposals }: Props): React.ReactElement<Props> | null {
  ...
  return isMember && !isVotingInitiated ? (
    <>
      <Button icon="step-forward" isDisabled={false} label={t<string>("Initiate voting")} onClick={toggleOpen} />
      {isOpen && (
        <Modal header={`${t<string>("Initiate voting")} - "${truncateTitle(description, 30)}"`} size="large">
          <Modal.Content>
            <Modal.Columns
              hint={t<string>(
