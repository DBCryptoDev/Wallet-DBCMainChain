  ...
  return (
    <>
      <tr className={`${className}${isExpanded ? " noBorder" : ""} ${isEven ? "isEven" : "isOdd"}`}>
        ...
        <td className="description-column" data-testid="description">
        ...
        <td>
          {curatorToRender && (
            <Curator curator={curatorToRender.curator} isFromProposal={curatorToRender.isFromProposal} />
          )}
        </td>
        <td>
          {blocksUntilPayout && unlockAt && (
            <DueBlocks dueBlocks={blocksUntilPayout} endBlock={unlockAt} label={t<string>("payout")} />
          )}
          {blocksUntilUpdate && updateDue && (
            <DueBlocks dueBlocks={blocksUntilUpdate} endBlock={updateDue} label={t<string>("update")} />
          )}
          <BountyActionMessage bestNumber={bestNumber} blocksUntilUpdate={blocksUntilUpdate} status={status} />
        </td>
        <td className="td-info-action-row">
          <div className="td-row">
            <BountyInfos beneficiary={beneficiary} proposals={proposals} status={status} />
        ...
        <td className="fast-actions">
          <div className="fast-actions-row">
            <LinkExternal data={index} isLogo type="bounty" />
            <BountyExtraActions
              bestNumber={bestNumber}
              description={description}
              index={index}
        ...
        <td>
          {proposals && (
            <div className="votes-table">
              <VotersColumn option={"ayes"} proposals={proposals} status={status} />
              <VotersColumn option={"nays"} proposals={proposals} status={status} />
            </div>
          )}
        </td>
        <td />
