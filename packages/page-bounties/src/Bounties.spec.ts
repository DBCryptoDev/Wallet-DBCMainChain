...
describe("Bounties", () => {
  ...
  describe("bounty in a list", () => {
    describe("has extended status", () => {
      it("when voting on proposed curator", async () => {
        ...
        const proposals = [
          aProposal(augmentedApi.tx.bounties.proposeCurator(0, "5EYCAe5ijiYfyeZ2JJCGq56LmPyNRAKzpG4QkoQkkQNB5e6Z", 1)),
        ];
    ...
    describe("has Beneficiary description", () => {
      it("in PendingPayout status", async () => {
        ...
        const proposals = [
          aProposal(augmentedApi.tx.bounties.awardBounty(0, "5EYCAe5ijiYfyeZ2JJCGq56LmPyNRAKzpG4QkoQkkQNB5e6Z")),
        ];
  ...
  describe("create bounty modal", () => {
    ...
    it("validates balance is enough for bond", async () => {
      bountiesPage.renderMany({ bountyDepositBase: new BN(10), dataDepositPerByte: new BN(1) }, { balance: 10 });
  ...
  describe("extend bounty expiry action modal", () => {
    it("queues extend bounty expiry extrinsic on submit", async () => {
      ...
      expect(mockBountyHooks.bountyApi.extendBountyExpiry).toHaveBeenCalledWith(
        aBountyIndex(0),
        "The bounty extend expiry remark"
      );
    });
  });
