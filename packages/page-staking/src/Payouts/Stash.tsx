...
function createPrevPayoutType(
  api: ApiPromise,
  { era, isValidator, nominating }: DeriveStakerReward
): SubmittableExtrinsic<"promise"> {
  return isValidator
    ? api.tx.staking.payoutValidator(era)
    : api.tx.staking.payoutNominator(
        era,
        nominating.map(({ validatorId, validatorIndex }): [string, number] => [validatorId, validatorIndex])
      );
}
...
function Stash({
  className = "",
  isDisabled,
  payout: { available, rewards, stashId },
  stakerPayoutsAfter,
  ...
  useEffect((): void => {
    if (rewards) {
      ...
      setExtrinsic(api.tx.utility && available.length ? createPrevPayout(api, available) : null);
    }
