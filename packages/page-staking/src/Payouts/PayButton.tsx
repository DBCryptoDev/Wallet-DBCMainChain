...
function createBatches(
  ...
  return payouts
    ...
    .reduce(
      (batches: SubmittableExtrinsic<"promise">[][], { era, validatorId }): SubmittableExtrinsic<"promise">[][] => {
    ...
    .map((batch) => (batch.length === 1 ? batch[0] : api.tx.utility.batch(batch)));
}

function createExtrinsics(
  api: ApiPromise,
  payout: PayoutValidator | PayoutValidator[],
  maxPayouts: number
  ...
  if (Array.isArray(payout)) {
    ...
    return createBatches(
      api,
      maxPayouts,
      payout.reduce((payouts: SinglePayout[], { eras, validatorId }): SinglePayout[] => {
  ...
  return eras.length === 1
    ...
    : createBatches(
        api,
        maxPayouts,
        eras.map((era): SinglePayout => ({ era: era.era, validatorId }))
      );
}
...
function PayButton({ className, isAll, isDisabled, payout }: Props): React.ReactElement<Props> {
  ...
  useEffect((): void => {
    if (api.tx.utility && allAccounts.length && payout && (!Array.isArray(payout) || payout.length !== 0)) {
      const { eras, validatorId } = Array.isArray(payout) ? payout[0] : payout;
      ...
      setMaxPayouts(Math.floor((36 * 64) / (api.consts.staking.maxNominatorRewardedPerValidator?.toNumber() || 64)));
    }
  }, [allAccounts, api, payout]);

  useEffect((): void => {
    api.tx.utility && payout && setExtrinsics(() => createExtrinsics(api, payout, maxPayouts));
  ...
  return (
    <>
      {payout && isVisible && (
        <Modal className={className} header={t<string>("Payout all stakers")} size="large">
          <Modal.Content>
            <Modal.Columns
              hint={t<string>(
            ...
            >
              {Array.isArray(payout) ? (
                <Static
                  ...
                  value={payout.map(({ validatorId }) => (
                    <AddressMini className="addressStatic" key={validatorId} value={validatorId} />
                  ))}
                />
              ) : (
                <InputAddress
      ...
      <Button
        ...
        label={isAll || Array.isArray(payout) ? t<string>("Payout all") : t<string>("Payout")}
