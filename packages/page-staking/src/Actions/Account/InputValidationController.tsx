...
const transformBonded = {
  transform: (value: Option<AccountId>): string | null => (value.isSome ? value.unwrap().toString() : null),
};

const transformStash = {
  transform: (value: Option<StakingLedger>): string | null => (value.isSome ? value.unwrap().stash.toString() : null),
};
...
function ValidateController({
  ...
  const { api } = useApi();
  const bondedId = useCall<string | null>(
    controllerId ? api.query.staking.bonded : null,
    [controllerId],
    transformBonded
  );
  const stashId = useCall<string | null>(
    controllerId ? api.query.staking.ledger : null,
    [controllerId],
    transformStash
  );
  ...
  return isFatal ? <MarkError content={error} /> : <MarkWarning content={error} />;
}
