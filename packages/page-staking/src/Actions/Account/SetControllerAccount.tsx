...
function SetControllerAccount({ defaultControllerId, onClose, stashId }: Props): React.ReactElement<Props> {
  ...
  const _setError = useCallback((_: string | null, isFatal: boolean) => setIsFatal(isFatal), []);

  return (
    <Modal header={t<string>("Change controller account")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={t<string>(
        ...
        >
          <InputAddress isDisabled label={t<string>("stash account")} value={stashId} />
        </Modal.Columns>
        <Modal.Columns
          hint={t<string>(
