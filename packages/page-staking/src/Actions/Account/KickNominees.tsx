...
function KickNominees({
  ...
  const nominators = useMemo(() => queryInfo?.exposure?.others.map(({ who }) => who.toString()), [queryInfo]);
  ...
  return (
    <Modal className={className} header={t<string>("Remove nominees")} size="large">
      <Modal.Content>
        <SenderInfo controllerId={controllerId} stashId={stashId} />
        {nominators ? (
          <InputAddressMulti
