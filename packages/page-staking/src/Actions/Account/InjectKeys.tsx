...
function InjectKeys({ onClose }: Props): React.ReactElement<Props> | null {
  ...
  const _cryptoOptions = useMemo(
    () =>
      CRYPTO_MAP[keyType].map((value): { text: string; value: KeypairType } => ({
        text: value === "ed25519" ? t<string>("ed25519, Edwards") : t<string>("sr15519, Schnorrkel"),
        value,
      })),
    [keyType, t]
  );
  ...
  return (
    <Modal header={t<string>("Inject Keys")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={t<string>(
        ...
        >
          <Input isDisabled label={t<string>("generated public key")} value={publicKey} />
        </Modal.Columns>
      </Modal.Content>
      <Modal.Actions onCancel={onClose}>
        <Button icon="sign-in-alt" label={t<string>("Submit key")} onClick={_onSubmit} />
      </Modal.Actions>
    </Modal>
  );
}
