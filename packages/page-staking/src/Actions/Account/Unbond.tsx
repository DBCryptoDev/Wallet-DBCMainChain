...
function Unbond({ className = "", controllerId, onClose, stakingLedger, stashId }: Props): React.ReactElement<Props> {
  ...
  return (
    <Modal className={`staking--Unbond ${className}`} header={t<string>("Unbond funds")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={t<string>("The stash and controller pair, here the controller will be used to send the transaction.")}
        >
          <InputAddress defaultValue={stashId} isDisabled label={t<string>("stash account")} />
          <InputAddress defaultValue={controllerId} isDisabled label={t<string>("controller account")} />
        </Modal.Columns>
        <Modal.Columns
          hint={t<string>(
        ...
        >
          <InputBalance
            ...
            withMax
          >
            <Toggle isOverlay label={t<string>("all bonded")} onChange={setWithMax} value={withMax} />
          </InputBalance>
          {bondedBlocks?.gtn(0) && (
            <Static
