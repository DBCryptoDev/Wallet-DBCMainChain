...
function SetRewardDestination({
  ...
  const [destination, setDestination] = useState<DestinationType>(
    () => ((defaultDestination?.isAccount ? "Account" : defaultDestination?.toString()) || "Staked") as "Staked"
  );
  const [destAccount, setDestAccount] = useState<string | null>(() =>
    defaultDestination?.isAccount ? defaultDestination.asAccount.toString() : null
  );
  ...
  return (
    <Modal header={t<string>("Bonding Preferences")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={t<string>(
            "The stash and controller pair as linked. This operation will be performed via the controller."
          )}
        >
          <InputAddress defaultValue={stashId} isDisabled label={t<string>("stash account")} />
          <InputAddress
        ...
        <Modal.Columns
          hint={t<string>("All rewards will go towards the selected output destination when a payout is made.")}
        >
          <Dropdown
      ...
      <Modal.Actions onCancel={onClose}>
        <TxButton
          ...
          onStart={onClose}
          params={[isAccount ? { Account: destAccount } : destination]}
