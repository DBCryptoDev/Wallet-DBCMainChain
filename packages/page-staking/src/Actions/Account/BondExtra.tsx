...
function calcBalance(api: ApiPromise, stakingInfo?: DeriveStakingAccount, stashBalance?: DeriveBalancesAll): BN | null {
  if (stakingInfo && stakingInfo.stakingLedger && stashBalance) {
    ...
    const available = stashBalance.freeBalance
      .sub(stakingInfo.stakingLedger.active.unwrap())
      .sub(sumUnlocking)
      .sub(redeemable);
...
function BondExtra({ controllerId, onClose, stakingInfo, stashId }: Props): React.ReactElement<Props> {
  ...
  const currentAmount = useMemo(() => stakingInfo && stakingInfo.stakingLedger?.active.unwrap(), [stakingInfo]);

  const startBalance = useMemo(() => calcBalance(api, stakingInfo, stashBalance), [api, stakingInfo, stashBalance]);

  return (
    <Modal className="staking--BondExtra" header={t<string>("Bond more funds")} size="large">
      <Modal.Content>
        ...
        {startBalance && (
          <Modal.Columns
            hint={t<string>("The amount placed at-stake should allow some free funds for future transactions.")}
          >
            <InputBalance
              autoFocus
              ...
              labelExtra={
                <BalanceFree label={<span className="label">{t<string>("balance")}</span>} params={stashId} />
              }
