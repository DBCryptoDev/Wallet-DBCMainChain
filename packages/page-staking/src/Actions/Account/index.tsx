...
function extractSlashes(stashId: string, allSlashes: [BN, UnappliedSlash[]][] = []): Slash[] {
  return allSlashes
    .map(([era, all]) => ({
      era,
      slashes: all.filter(
        ({ others, validator }) => validator.eq(stashId) || others.some(([nominatorId]) => nominatorId.eq(stashId))
      ),
    }))
  ...
  return (
    <tr className={className}>
      ...
      <td className="address">
        ...
        {isInjectOpen && <InjectKeys onClose={toggleInject} />}
        {isKickOpen && controllerId && (
          <KickNominees controllerId={controllerId} onClose={toggleKick} stashId={stashId} />
        )}
        ...
        {isSetControllerOpen && controllerId && (
          <SetControllerAccount defaultControllerId={controllerId} onClose={toggleSetController} stashId={stashId} />
        )}
        ...
        {isSetSessionOpen && controllerId && (
          <SetSessionKey controllerId={controllerId} onClose={toggleSetSession} stashId={stashId} />
        )}
        {isUnbondOpen && (
          <Unbond controllerId={controllerId} onClose={toggleUnbond} stakingLedger={stakingLedger} stashId={stashId} />
        )}
        {isValidateOpen && controllerId && (
          <Validate controllerId={controllerId} onClose={toggleValidate} stashId={stashId} />
        )}
      </td>
      <td className="address">
      ...
      <td className="start media--1200">
        {destination?.isAccount ? <AddressMini value={destination.asAccount} /> : destination?.toString()}
      </td>
      <td className="number">
      ...
      {isStashValidating ? (
        ...
        <td className="all expand left">
          {isStashNominating && <ListNominees nominating={nominating} stashId={stashId} />}
        </td>
      )}
      <td className="button">
        {!isLoading && (
          <>
            ...
            <Popup
              ...
              trigger={<Button icon="ellipsis-v" isDisabled={isDisabled} onClick={toggleSettings} />}
            >
              <Menu onClick={toggleSettings} text vertical>
                <Menu.Item disabled={!isOwnStash || !balancesAll?.freeBalance.gtn(0)} onClick={toggleBondExtra}>
                ...
                <Menu.Divider />
                <Menu.Item disabled={!isOwnStash} onClick={toggleSetController}>
                  {t<string>("Change controller account")}
                </Menu.Item>
                <Menu.Item disabled={!isOwnController} onClick={toggleRewardDestination}>
                  {t<string>("Change reward destination")}
                </Menu.Item>
                {isStashValidating && (
                  <>
                    <Menu.Item disabled={!isOwnController} onClick={toggleValidate}>
                      {t<string>("Change validator preferences")}
                    </Menu.Item>
                    {isFunction(api.tx.staking.kick) && (
                      <Menu.Item disabled={!isOwnController} onClick={toggleKick}>
                ...
                <Menu.Divider />
                {!isStashNominating && (
                  <Menu.Item disabled={!isOwnController} onClick={toggleSetSession}>
                    {t<string>("Change session keys")}
                  </Menu.Item>
                )}
                {isStashNominating && (
                  <Menu.Item disabled={!isOwnController || !targets.validators?.length} onClick={toggleNominate}>
                    {t<string>("Set nominees")}
                  </Menu.Item>
                )}
                {!isStashNominating && (
                  <Menu.Item onClick={toggleInject}>{t<string>("Inject session keys (advanced)")}</Menu.Item>
                )}
              </Menu>
            </Popup>
          </>
