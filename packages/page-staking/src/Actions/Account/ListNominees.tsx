...
function renderNominators(
  stashId: string,
  all: string[] = [],
  eraExposure?: DeriveEraExposure
): null | [number, () => React.ReactNode[]] {
  return all.length
    ? [
        all.length,
        (): React.ReactNode[] => {
...
function ListNominees({ nominating, stashId }: Props): React.ReactElement<Props> {
  ...
  const eraExposure = useCall<DeriveEraExposure>(
    isFunction(api.query.staking.erasStakers) && api.derive.staking.eraExposure,
  ...
  const [renActive, renChilled, renInactive, renOver, renWaiting] = useMemo(
    () => [
      ...
      renderNominators(stashId, nomsChilled),
    ...
    <>
      {renOver && (
        <Expander
          className="stakeOver"
          help={t<string>(
            "These validators are active but only the top {{max}} nominators by backing stake will be receiving rewards. The nominating stash is not one of those to be rewarded in the current era.",
            { replace: api.consts.staking?.maxNominatorRewardedPerValidator?.toString() }
          )}
      ...
      {renInactive && (
        <Expander
          help={t<string>(
            "The elected validator list that did not get selected by the Phragmen algorithm for this era. However they may be selected in the future."
          )}
      ...
      {renChilled && (
        <Expander
          help={t<string>(
            "The validators that got slashed and for which your nomination got auto-chilled. Re-nominating these will make them available to the Phragmen algorithm."
          )}
      ...
      {renWaiting && (
        <Expander
          help={t<string>(
            "The validators that are not in the validator set because they need more nominations or because they have willingly stop validating. Any nominations made before the next election will also appear here."
          )}
      ...
      {nomsActive && nomsInactive && nomsActive.length === 0 && nomsInactive.length !== 0 && (
        <MarkWarning
          content={t<string>(
            "Your nomination has not been applied to any validator in the active set by the election algorithm. This could mean that all your validators are over-subscribed or that you have less bonded than the lowest nominator elected for each of the validators."
          )}
        />
      )}
    </>
