...
function NewValidator({ isInElection }: Props): React.ReactElement<Props> {
  ...
  const _toggle = useCallback((): void => {
    setBondInfo({});
    setSessionInfo({});
    setValidateInfo({});
    setStep(1);
  ...
  return (
    <>
      ...
      {isVisible && (
        ...
        >
          <Modal.Content>
            {step === 1 && <BondPartial onChange={setBondInfo} />}
            {controllerId && stashId && step === 2 && (
              <>
                <SessionKeyPartial controllerId={controllerId} onChange={setSessionInfo} stashId={stashId} />
                <ValidatePartial controllerId={controllerId} onChange={setValidateInfo} stashId={stashId} />
              </>
            )}
            <Modal.Columns>
              <BatchWarning />
          ...
          <Modal.Actions onCancel={_toggle}>
            <Button icon="step-backward" isDisabled={step === 1} label={t<string>("prev")} onClick={_prevStep} />
            {step === NUM_STEPS ? (
              <TxButton
                accountId={stashId}
                icon="sign-in-alt"
              ...
              <Button icon="step-forward" isDisabled={!bondTx} label={t<string>("next")} onClick={_nextStep} />
            )}
          </Modal.Actions>
        </Modal>
      )}
