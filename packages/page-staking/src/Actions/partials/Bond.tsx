...
function Bond({ className = "", isNominating, minNomination, onChange }: Props): React.ReactElement<Props> {
  ...
  const _setError = useCallback((_: string | null, isFatal: boolean) => setControllerError(isFatal), []);
  ...
  useEffect((): void => {
    const bondDest = destination === "Account" ? { Account: destAccount } : destination;
  ...
  return (
    <div className={className}>
      ...
      >
        <InputAddress label={t<string>("stash account")} onChange={setStashId} type="account" value={stashId} />
        <InputAddress
          help={t<string>(
          ...
          value={controllerId}
        />
        <InputValidationController accountId={stashId} controllerId={controllerId} onError={_setError} />
      </Modal.Columns>
      {startBalance && (
        <Modal.Columns
          hint={
        ...
        >
          <InputBalance
            ...
            help={t<string>(
              "The total amount of the stash balance that will be at stake in any forthcoming rounds (should be less than the free amount available)"
            )}
            isError={!hasValue || !!amountError?.error}
            label={t<string>("value bonded")}
            labelExtra={<BalanceFree label={<span className="label">{t<string>("balance")}</span>} params={stashId} />}
            onChange={setAmount}
          />
          <InputValidateAmount
          ...
          {bondedBlocks?.gtn(0) && (
            <Static
              help={t<string>(
                "The bonding duration for any staked funds. Needs to be unlocked and withdrawn to become available."
              )}
