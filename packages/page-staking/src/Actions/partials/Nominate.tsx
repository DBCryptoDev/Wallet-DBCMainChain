...
function Nominate({
  ...
  const [available] = useState<string[]>((): string[] => {
    ...
    return shortlist.concat(...nominateIds.filter((acc) => !shortlist.includes(acc)));
  });
  ...
  return (
    <div className={className}>
      {withSenders && <SenderInfo controllerId={controllerId} stashId={stashId} />}
      <Modal.Columns
        hint={
          <>
            <p>
              {t<string>("Nominators can be selected manually from the list of all currently available validators.")}
            </p>
            <p>
              {t<string>(
