...
function Validator({
  ...
  const isVisible = useMemo(
    () => (accountInfo ? checkVisibility(api, info.key, accountInfo, filterName) : true),
  ...
  return (
    <tr>
      <td className="badge together">
        <Favorite address={key} isFavorite={isFavorite} toggleFavorite={toggleFavorite} />
        {isNominated ? <Badge color="green" icon="hand-paper" /> : <Badge color="transparent" />}
        {isElected ? <Badge color="blue" icon="chevron-right" /> : <Badge color="transparent" />}
        <MaxBadge numNominators={numNominators || nominatedBy.length} />
        {isBlocking && <Badge color="red" icon="user-slash" />}
        {slashes.length !== 0 && (
          <Badge
            color="red"
      ...
      <td>{!isBlocking && (canSelect || isSelected) && <Checkbox onChange={_toggleSelected} value={isSelected} />}</td>
      <td>
        <Icon className="staking--stats highlight--color" icon="chart-line" onClick={_onQueryStats} />
      </td>
    </tr>
  );
}
