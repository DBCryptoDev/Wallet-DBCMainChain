...
function applyFilter(
  ...
  medianComm: number,
  allIdentity: Record<string, DeriveHasIdentity>,
  { daysPayout, isBabe, maxPaid, withElected, withGroup, withIdentity, withPayout, withoutComm, withoutOver }: Flags,
  nominatedBy?: Record<string, NominatedBy[]>
): ValidatorInfo[] {
  const displays: string[][] = [];
  const parentIds: string[] = [];

  return validators.filter(
    ({ accountId, commissionPer, isElected, isFavorite, lastPayout, numNominators }): boolean => {
      if (isFavorite) {
        return true;
      }
      ...
      if (
        ...
        (!withoutComm ||
          (MAX_COMM_PERCENT > 0 ? commissionPer < MAX_COMM_PERCENT : !medianComm || commissionPer <= medianComm)) &&
...
function sort(sortBy: TargetSortBy, sortFromMax: boolean, validators: ValidatorInfo[]): ValidatorInfo[] {
  return validators
    .sort((a, b) => (sortFromMax ? a[sortBy] - b[sortBy] : b[sortBy] - a[sortBy]))
...
function Targets({
  ...
  const _sort = useCallback(
    (newSortBy: TargetSortBy) =>
      setSortBy(({ sortBy, sortFromMax }) => ({
        sortBy: newSortBy,
        sortFromMax: newSortBy === sortBy ? !sortFromMax : true,
      })),
    []
  );
  ...
  const _toggleSelected = useCallback(
    (address: string) =>
      setSelected(
        selected.includes(address) ? selected.filter((accountId) => address !== accountId) : [...selected, address]
      ),
    [selected]
  );

  const _selectProfitable = useCallback(() => filtered && setSelected(selectProfitable(filtered)), [filtered]);
  ...
  const header = useMemo(
    () => [
      ...
      ...(SORT_KEYS as (keyof typeof labelsRef.current)[]).map((header) => [
        <>
          {labelsRef.current[header]}
          <Icon icon={sortBy === header ? (sortFromMax ? "chevron-down" : "chevron-up") : "minus"} />
        </>,
        `${sorted ? `isClickable ${sortBy === header ? "highlight--border" : ""} number` : "number"} ${
          CLASSES[header] || ""
        }`,
        1,
  ...
  return (
    <div className={className}>
      ...
      <Button.Group>
        ...
        />
        <Nominate isDisabled={isInElection || !validators?.length} ownNominators={ownNominators} targets={selected} />
      </Button.Group>
