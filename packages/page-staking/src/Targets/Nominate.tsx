...
function Nominate({ className = "", isDisabled, ownNominators, targets }: Props): React.ReactElement<Props> {
  ...
  const stashes = useMemo(() => (ownNominators || []).map(({ stashId }) => stashId), [ownNominators]);

  const _onChangeStash = useCallback(
    (accountId?: string | null): void => {
      const acc = ownNominators && ownNominators.find(({ stashId }) => stashId === accountId);

      setIds(acc ? { controllerId: acc.controllerId, stashId: acc.stashId } : null);
    },
    [ownNominators]
  );
          ...
          <Modal.Content>
            ...
            >
              <InputAddress
                ...
                value={ids?.stashId}
              />
              <InputAddress isDisabled label={t<string>("the associated controller")} value={ids?.controllerId} />
            </Modal.Columns>
            <Modal.Columns
              hint={
                <>
