    ...
    <SummaryBox>
      <section className="media--800">
        {totalIssuance && totalStaked?.gt(BN_ZERO) && (
          <CardSummary label={t<string>("total staked")} progress={progressStake}>
      ...
      <section className="media--800">
        {totalIssuance && stakedReturn > 0 && Number.isFinite(stakedReturn) && (
          <CardSummary label={t<string>("returns")}>{stakedReturn.toFixed(1)}%</CardSummary>
        )}
      </section>
      <section className="media--1000">
        {avgStaked?.gtn(0) && lowStaked?.gtn(0) && (
          <CardSummary label={`${t<string>("lowest / avg staked")}`} progress={progressAvg}>
      ...
      <section className="media--1600">
        {minNominated?.gt(BN_ZERO) && (
          <CardSummary className="media--1600" label={t<string>("min nominated")}>
