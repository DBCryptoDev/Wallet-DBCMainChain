...
function extractSingle(
  ...
  favorites: string[],
  { activeEra, eraLength, lastEra, sessionLength }: LastEra,
  historyDepth?: BN
  ...
  const list = derive.info.map(
    ({ accountId, exposure = emptyExposure, stakingLedger, validatorPrefs }): ValidatorInfo => {
      ...
      const minNominated = (exposure.others || []).reduce(
        (min: BN, { value = api.createType("Compact<Balance>") }): BN => {
      ...
      let lastPayout: BN | undefined =
        earliestEra && lastEraPayout && lastEraPayout.gt(earliestEra) ? lastEraPayout : undefined;
