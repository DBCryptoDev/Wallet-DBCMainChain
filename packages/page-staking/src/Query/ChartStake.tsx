...
function ChartStake({ validatorId }: Props): React.ReactElement<Props> {
  ...
  const { chart, labels } = useMemo(() => extractStake(ownExposures, divisor), [divisor, ownExposures]);
  ...
  return (
    <div className="staking--Chart">
      ...
      {labels.length ? (
        <Chart.Line colors={COLORS_STAKE} labels={labels} legends={legends} values={chart} />
      ) : (
        <Spinner />
      )}
    </div>
