...
function Row({
  ...
  slash: {
    isMine,
    slash: { others, own, payout, reporters, validator },
    total,
    totalOther,
  },
  ...
  return (
    <tr>
      <td className="badge">{isMine && <Badge color="red" icon="skull-crossbones" />}</td>
      ...
      <td className="expand all">
        {!!others.length && (
          <Expander summary={t<string>("Nominators ({{count}})", { replace: { count: formatNumber(others.length) } })}>
            {others.map(
              ([accountId, balance], index): React.ReactNode => (
                <AddressMini balance={balance} key={index} value={accountId} withBalance />
              )
            )}
          </Expander>
        )}
      ...
      <td className="address">
        {reporters.map(
          (reporter, index): React.ReactNode => (
            <AddressMini key={index} value={reporter} />
          )
        )}
      </td>
      ...
      <td>
        <Checkbox isDisabled={!onSelect} onChange={_onSelect} value={isSelected} />
      </td>
    </tr>
  );
}
