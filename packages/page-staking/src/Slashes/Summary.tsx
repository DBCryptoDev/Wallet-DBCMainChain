...
function Header({ slash: { era, nominators, reporters, total, validators } }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <SummaryBox>
      <section>
        <CardSummary label={t<string>("validators")}>{formatNumber(validators.length)}</CardSummary>
        <CardSummary label={t<string>("nominators")}>{formatNumber(nominators.length)}</CardSummary>
        <CardSummary label={t<string>("reporters")}>{formatNumber(reporters.length)}</CardSummary>
      </section>
      {blockProgress.gtn(0) && (
        <CardSummary
