...
function extractFunction(all: string[]): null | [number, () => React.ReactNode[]] {
  return all.length
    ? [all.length, () => all.map((who): React.ReactNode => <AddressMini key={who} value={who} />)]
    : null;
}
...
function extractChilled(nominators: NominatedByType[] = [], slashingSpans?: SlashingSpans | null): Chilled {
  const chilled = slashingSpans
    ? nominators
        .filter(
          ({ submittedIn }) =>
            !slashingSpans.lastNonzeroSlash.isZero() && slashingSpans.lastNonzeroSlash.gte(submittedIn)
        )
  ...
  return {
    active: extractFunction(
      nominators.filter(({ nominatorId }) => !chilled.includes(nominatorId)).map(({ nominatorId }) => nominatorId)
    ),
...
function NominatedBy({ nominators, slashingSpans }: Props): React.ReactElement<Props> {
  ...
  const { active, chilled } = useMemo(() => extractChilled(nominators, slashingSpans), [nominators, slashingSpans]);
