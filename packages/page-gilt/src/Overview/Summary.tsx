...
function Summary({ activeTotal, className, isDisabled }: Props): React.ReactElement<Props> {
  ...
  return (
    <SummaryBox className={className}>
      <section>
        <CardSummary label={t<string>("active")}>{isDisabled ? t("no") : t("yes")}</CardSummary>
        {activeTotal && <CardSummary label={t<string>("index")}>{formatNumber(activeTotal.index)}</CardSummary>}
      </section>
      {activeTotal && (
        <section>
