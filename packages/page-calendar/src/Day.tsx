...
function Day({
  ...
  const viewSetter = useCallback(
    () => <Button className="all-events-button" icon={"list"} onClick={() => setView(true)} />,
    [setView]
  );
  ...
  return (
    <div className={className}>
      <h1>
        ...
        <Button.Group>
          <Button icon="chevron-left" isDisabled={isToday} onClick={setPrevDay} />
          <Button icon="chevron-right" isDisabled={!hasNextDay} onClick={setNextDay} />
        </Button.Group>
      </h1>
      <div className="hoursContainer highlight--bg-faint">
        {HOURS.map(
          (hour, index): React.ReactNode => (
            <DayHour
              date={date}
              hour={hour}
              index={index}
