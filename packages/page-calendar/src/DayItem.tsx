...
function DayItem({ className, item: { date, info, type }, showAllEvents }: Props): React.ReactElement<Props> {
  ...
  const desc = useMemo((): React.ReactNode => {
    const id: string | null = info && (isString(info) ? info : formatNumber(info));
    const typeLink = ["councilElection"].includes(type) ? (
      <div className="itemLink">
        <a href="#/council">{t<string>("via Council")}</a>
      </div>
    ) : ["councilMotion"].includes(type) ? (
      <div className="itemLink">
        <a href="#/council/motions">{t<string>("via Council/Motions")}</a>
      </div>
    ) : ["democracyDispatch", "scheduler"].includes(type) ? (
      <div className="itemLink">
        <a href="#/democracy/dispatch">{t<string>("via Democracy/Dispatch")}</a>
      </div>
    ) : ["democracyLaunch", "referendumDispatch", "referendumVote"].includes(type) ? (
      <div className="itemLink">
        <a href="#/democracy">{t<string>("via Democracy")}</a>
      </div>
    ) : ["societyChallenge", "societyRotate"].includes(type) ? (
      <div className="itemLink">
        <a href="#/society">{t<string>("via Society")}</a>
      </div>
    ) : ["stakingEpoch", "stakingEra"].includes(type) ? (
      <div className="itemLink">
        <a href="#/staking">{t<string>("via Staking")}</a>
      </div>
    ) : ["stakingSlash"].includes(type) ? (
      <div className="itemLink">
        <a href="#/staking/slashes">{t<string>("via Staking/Slashed")}</a>
      </div>
    ) : ["treasurySpend"].includes(type) ? (
      <div className="itemLink">
        <a href="#/treasury">{t<string>("via Treasury")}</a>
      </div>
    ) : undefined;
    let s = "";
  ...
  return (
    <div className={className}>
      {showAllEvents && <div className="itemDate">{date.toLocaleString(undefined, FORMAT_OPTIONS)}</div>}
