...
function Candidates({ allVotes = {}, electionsInfo }: Props): React.ReactElement<Props> {
  ...
  return (
    <>
      <Table empty={electionsInfo && t<string>("No runners up found")} header={headerRunnersRef.current}>
        {electionsInfo?.runnersUp.map(
      ...
      <Table empty={electionsInfo && t<string>("No candidates found")} header={headerCandidatesRef.current}>
        {electionsInfo?.candidates.map(
          (accountId): React.ReactNode => (
            <Candidate address={accountId} key={accountId.toString()} voters={allVotes[accountId.toString()]} />
          )
        )}
      </Table>
    </>
