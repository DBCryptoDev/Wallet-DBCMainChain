...
function Vote({ electionsInfo }: Props): React.ReactElement<Props> {
  ...
  useEffect((): void => {
    accountId &&
      api.derive.council.votesOf(accountId).then(({ votes }): void => {
        setDefaultVotes(
          votes
            .map((accountId): string => accountId.toString())
  ...
  return (
    <>
      <Button icon="check" isDisabled={available.length === 0} label={t<string>("Vote")} onClick={toggleVisible} />
      {isVisible && (
        <Modal header={t<string>("Vote for current candidates")} size="large">
          <Modal.Content>
            ...
            >
              <VoteValue accountId={accountId} isCouncil onChange={setVoteValue} />
            </Modal.Columns>
            <Modal.Columns
              hint={
                <>
