    ...
    <div className={className}>
      <Button.Group>
        {/* <ProposeMotion
          ...
          members={members}
        /> */}
        <ProposeExternal isMember={isMember} members={members} />
        <Slashing isMember={isMember} members={members} />
      </Button.Group>
      <Table empty={motions && t<string>("No council motions")} header={headerRef.current}>
        {motions?.map(
          (motion: DeriveCollectiveProposal): React.ReactNode => (
            <Motion isMember={isMember} key={motion.hash.toHex()} members={members} motion={motion} prime={prime} />
          )
        )}
      </Table>
    </div>
