...
function Propose({ isMember, members }: Props): React.ReactElement<Props> {
  ...
  return (
    <>
      <Button icon="plus" isDisabled={!isMember} label={t<string>("Propose motion")} onClick={toggleOpen} />
      {isOpen && (
        <Modal header={t<string>("Propose a council motion")} size="large">
          <Modal.Content>
            <Modal.Columns
              hint={t<string>(
            ...
            >
              <InputNumber
                ...
                placeholder={t<string>("Positive number between 1 and {{memberCount}}", {
            ...
            <Modal.Columns
              hint={t<string>("The actual proposal to make, based on the selected call and parameters thereof.")}
            >
              <Extrinsic defaultValue={apiDefaultTxSudo} label={t<string>("proposal")} onChange={_setMethod} />
            </Modal.Columns>
          </Modal.Content>
