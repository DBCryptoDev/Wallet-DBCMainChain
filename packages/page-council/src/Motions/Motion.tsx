...
function Motion({
  ...
  return (
    <tr className={className}>
      ...
      <ProposalCell imageHash={hash} proposal={proposal} />
      <td className="number together">{formatNumber(threshold)}</td>
      <td className="number together">
        {remainingBlocks && end && (
          <>
            <BlockToTime value={remainingBlocks} />#{formatNumber(end)}
          </>
        )}
      </td>
      <td className="expand">
        <Voters isAye members={members} threshold={threshold} votes={ayes} />
        <Voters members={members} threshold={threshold} votes={nays} />
      </td>
      <td className="button">
        ...
        {isCloseable &&
          (isMultiMembers ? (
            <Close hasFailed={hasFailed} hash={hash} idNumber={index} members={members} proposal={proposal} />
          ) : (
            <TxButton
              accountId={councilId}
              icon="times"
      ...
      <td className="badge">
        {isMember && <Icon color={hasVoted ? (hasVotedAye ? "green" : "red") : "gray"} icon="asterisk" />}
      </td>
      <td className="links">
        <LinkExternal data={index} hash={hash.toString()} isLogo type="council" />
      </td>
    </tr>
  );
}
