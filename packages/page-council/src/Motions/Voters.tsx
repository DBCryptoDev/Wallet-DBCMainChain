...
function Voters({ isAye, members, threshold, votes }: Props): React.ReactElement<Props> | null {
  ...
  const count = useMemo((): string => {
    ...
    const max = isAye ? num : members?.length ? members.length - num + 1 : 0;
  ...
  return (
    <Expander
      summary={
        isAye ? t<string>("Aye {{count}}", { replace: { count } }) : t<string>("Nay {{count}}", { replace: { count } })
      }
    >
      {votes.map(
        (address): React.ReactNode => (
          <AddressMini key={address.toString()} value={address} withBalance={false} />
        )
      )}
    </Expander>
  );
