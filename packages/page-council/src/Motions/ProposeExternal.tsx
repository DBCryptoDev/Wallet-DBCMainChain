...
function ProposeExternal({ className = "", isMember, members }: Props): React.ReactElement<Props> {
  ...
  return (
    <>
      <Button icon="plus" isDisabled={!isMember} label={t<string>("Propose external")} onClick={toggleVisible} />
      {isVisible && (
        <Modal className={className} header={t<string>("Propose external (majority)")} size="large">
          <Modal.Content>
            <Modal.Columns
              hint={t<string>(
