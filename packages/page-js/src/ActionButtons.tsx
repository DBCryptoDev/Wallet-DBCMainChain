...
function ActionButtons({
  ...
  const _onChangeName = useCallback((snippetName: string) => setSnippetName(snippetName), []);

  const _onPopupOpen = useCallback(() => setIsOpen(true), []);

  const _onPopupClose = useCallback((): void => {
    setSnippetName("");
    setIsOpen(false);
  }, []);

  const _saveSnippet = useCallback((): void => {
  ...
  return (
    <Button.Group className={`${className} action-button`}>
      {isCustomExample ? (
        <Button icon="trash" onClick={removeSnippet} />
      ) : (
        <Popup
          ...
          trigger={<Button icon="save" onClick={_onPopupOpen} />}
        >
          <Input
            autoFocus
            maxLength={50}
      ...
      {isRunning ? (
        <Button icon="times" onClick={stopJs} />
      ) : (
        <Button className="play-button" icon="play" onClick={runJs} />
      )}
    </Button.Group>
  );
}
