...
function DispatchQueue({ className }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <Table className={className} empty={queued && t<string>("Nothing queued for execution")} header={headerRef.current}>
      {queued?.map(
        (entry): React.ReactNode => (
          <DispatchEntry key={entry.index.toString()} value={entry} />
        )
      )}
    </Table>
  );
