...
function Propose({ className = "", onClose }: Props): React.ReactElement<Props> {
  ...
  const _onChangeHash = useCallback((hash?: string): void => setHash({ hash, isHashValid: isHex(hash, 256) }), []);

  const hasMinLocked = balance?.gte(api.consts.democracy.minimumDeposit);

  return (
    <Modal className={className} header={t<string>("Submit proposal")} size="large">
      <Modal.Content>
        <Modal.Columns
          hint={t<string>(
        ...
        >
          <InputAddress
            ...
            labelExtra={
              <Available label={<span className="label">{t<string>("transferrable")}</span>} params={accountId} />
            }
        ...
        <Modal.Columns
          hint={t<string>("The hash of the preimage for the proposal as previously submitted or intended.")}
        >
          <Input
            autoFocus
      ...
      <Modal.Actions onCancel={onClose}>
        <TxButton
          ...
          onStart={onClose}
          params={
            api.tx.democracy.propose.meta.args.length === 3 ? [hash, balance, publicProps?.length] : [hash, balance]
          }
