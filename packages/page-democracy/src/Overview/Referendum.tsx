...
function Referendum({
  ...
  const [percentages, { hasVoted, hasVotedAye }] = useMemo((): [Percentages | null, VoteType] => {
    if (totalIssuance) {
  ...
  return (
    <tr className={className}>
      ...
      <ProposalCell imageHash={imageHash} proposal={image?.proposal} />
      ...
      <td className="number together media--1400">
        <BlockToTime value={enactBlock.sub(bestNumber)} />#{formatNumber(enactBlock)}
      </td>
      <td className="number together media--1400">
        {percentages && (
          <>
      ...
      <td className="button">
        <Button.Group>
          {!image?.proposal && <PreImageButton imageHash={imageHash} />}
          <Voting proposal={image?.proposal} referendumId={index} />
        </Button.Group>
      </td>
      <td className="badge">
        <Icon color={hasVoted ? (hasVotedAye ? "green" : "red") : "gray"} icon="asterisk" />
      </td>
      <td className="links media--1000">
        <LinkExternal data={index} isLogo type="referendum" />
      </td>
    </tr>
  );
}
