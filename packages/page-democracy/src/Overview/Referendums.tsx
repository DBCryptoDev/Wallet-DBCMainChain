...
function Referendums({ className = "", referendums }: Props): React.ReactElement<Props> {
  ...
  return (
    <Table className={className} empty={referendums && t<string>("No active referendums")} header={headerRef.current}>
      {referendums?.map(
        (referendum): React.ReactNode => (
          <Referendum key={referendum.index.toString()} value={referendum} />
        )
      )}
    </Table>
  );
