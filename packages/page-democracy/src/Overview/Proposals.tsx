...
function Proposals({ className }: Props): React.ReactElement<Props> {
  ...
  return (
    <Table className={className} empty={proposals && t<string>("No active proposals")} header={headerRef.current}>
      {proposals?.map(
        (proposal): React.ReactNode => (
          <ProposalDisplay key={proposal.index.toString()} value={proposal} />
        )
      )}
    </Table>
  );
