...
function Fasttrack({ imageHash, threshold }: Props): React.ReactElement<Props> | null {
  ...
  const memberThreshold = useMemo(() => Math.ceil(members.length * 0.66), [members]);
  ...
  return (
    <>
      {isFasttrackOpen && (
        <Modal header={t<string>("Fast track proposal")} size="small">
          <Modal.Content>
            <InputAddress
              filter={members}
      ...
      <Button icon="fast-forward" isDisabled={!allowFast} label={t<string>("Fast track")} onClick={toggleFasttrack} />
    </>
  );
}
