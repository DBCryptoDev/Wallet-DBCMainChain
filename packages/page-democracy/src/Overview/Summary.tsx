...
function Summary({ referendumCount }: Props): React.ReactElement<Props> {
  ...
  const [publicPropCount, referendumTotal] = useCallMulti<[BN | undefined, BN | undefined]>(
    [api.query.democracy.publicPropCount, api.query.democracy.referendumCount],
    optMulti
  );
  ...
  return (
    <SummaryBox>
      <section>
        <CardSummary label={t<string>("proposals")}>{formatNumber(activeProposals?.length)}</CardSummary>
        <CardSummary label={t<string>("total")}>{formatNumber(publicPropCount)}</CardSummary>
      </section>
      <section>
        <CardSummary label={t<string>("referenda")}>{formatNumber(referendumCount || 0)}</CardSummary>
        <CardSummary label={t<string>("total")}>{formatNumber(referendumTotal || 0)}</CardSummary>
      </section>
      {bestNumber && (
