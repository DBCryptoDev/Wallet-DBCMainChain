...
function Seconding({ deposit, depositors, image, proposalId }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <>
      {isSecondingOpen && (
        <Modal header={t<string>("Second proposal")} size="large">
          <Modal.Content>
            <Modal.Columns
              hint={t<string>(
            ...
            >
              <ProposedAction idNumber={proposalId} proposal={image?.proposal} />
            </Modal.Columns>
            <Modal.Columns
              hint={t<string>(
                "Seconding a proposal that indicates your backing for the proposal. Proposals with greater interest moves up the queue for potential next referendums."
              )}
            >
              <InputAddress
                help={t<string>(
      ...
      <Button icon="toggle-off" label={t<string>("Second")} onClick={toggleSeconding} />
    </>
  );
}
