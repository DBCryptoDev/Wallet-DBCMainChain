...
function ProposalCell({ className = "", imageHash, proposal }: Props): React.ReactElement<Props> {
  ...
  return (
    <td className={`${className} all`}>
      <CallExpander labelHash={t<string>("proposal hash")} value={proposal} withHash={!isTreasury && !isExternal}>
        {isExternal && <ExternalCell value={proposal.args[0] as Hash} />}
        {isTreasury && <TreasuryCell value={proposal.args[0] as Compact<ProposalIndex>} />}
      </CallExpander>
    </td>
  );
}
