...
function PreImage({ className = "", imageHash, isImminent = false, onClose }: Props): React.ReactElement<Props> {
  ...
  const [{ encodedHash, encodedProposal, storageFee }, setHash] = useState<HashState>({
  ...
  useEffect((): void => {
    ...
    const storageFee = api.consts.democracy.preimageByteDeposit.mul(
      encodedProposal ? new BN((encodedProposal.length - 2) / 2) : BN_ZERO
    );
  ...
  return (
    <Modal className={className} header={t<string>("Submit preimage")} size="large">
      <Modal.Content>
        ...
        >
          <Extrinsic defaultValue={apiDefaultTxSudo} label={t<string>("propose")} onChange={setProposal} />
          <Input
