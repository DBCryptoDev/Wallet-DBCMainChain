...
function getDiffs(votes: BN, total: BN, change: BN, inc: BN, totalInc: 0 | 0.1 | 1, direction: 1 | -1): [BN, BN, BN] {
  ...
  return [voteChange, votes.add(multiplier.mul(voteChange)), BN.max(BN_ZERO, total.add(multiplier.mul(totalChange)))];
}
...
export function approxChanges(threshold: VoteThreshold, sqrtElectorate: BN, state: ApproxState): Approx {
  ...
  return {
    changeAye: state.votedNay.isZero() ? BN_ZERO : isPassing ? BN.min(changeAye, state.votedAye) : changeAye,
    changeNay: state.votedAye.isZero() ? BN_ZERO : isPassing ? changeNay : BN.min(changeNay, state.votedNay),
  };
}
