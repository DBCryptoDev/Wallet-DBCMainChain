...
describe("approxChanges", (): void => {
  it("approximates where the points are", (): void => {
    ...
    console.error(
      ...
      "changeAye",
      fmtBn(changeAye),
      fmtBn(ACTUAL.votedAye.sub(changeAye)),
      "\n",
      "changeNay",
      fmtBn(changeNay),
      fmtBn(ACTUAL.votedNay.add(changeNay))
    );

    expect(calcPassing(threshold, ACTUAL.sqrtElectorate, { ...ACTUAL, votedAye: ACTUAL.votedAye.sub(changeAye) })).toBe(
      false
    );
    expect(calcPassing(threshold, ACTUAL.sqrtElectorate, { ...ACTUAL, votedNay: ACTUAL.votedNay.add(changeNay) })).toBe(
      false
    );
  });
});
