...
function Bid({ auctionInfo, className, lastWinners, ownedIds }: Props): React.ReactElement<Props> {
  ...
  const rangeOpts = useMemo((): Option[] => {
  ...
  const currentWinner = useMemo(
    () =>
      lastWinners &&
      lastWinners.winners.find(
        ({ firstSlot, lastSlot }) =>
          firstSlot.eqn(rangeOpts[range].firstSlot) && lastSlot.eqn(rangeOpts[range].lastSlot)
      ),
    ...
    <>
      <Button
        ...
        isDisabled={
          ...
          bestNumber?.gte(auctionInfo.endBlock.add(api.consts.auctions.endingPeriod as BlockNumber))
        }
        label={t<string>("Bid")}
        onClick={toggleOpen}
      />
      {isOpen && (
        <Modal className={className} header={t<string>("Place bid")} size="large">
          <Modal.Content>
            <InputOwner onChange={setOwnerInfo} ownedIds={ownedIds} />
            <Modal.Columns
              hint={t<string>(
                "The first and last lease period for this bid. The last lease period should be after the first and a maximum of 3 more than the first."
              )}
            >
              <Dropdown
          ...
          <Modal.Actions onCancel={toggleOpen}>
            <TxButton
              ...
              params={[
                paraId,
                auctionInfo?.numAuctions,
                auctionInfo?.leasePeriod?.addn(RANGES[range][0]),
                auctionInfo?.leasePeriod?.addn(RANGES[range][1]),
                amount,
              ]}
