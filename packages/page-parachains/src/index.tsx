...
function ParachainsApp({ basePath, className }: Props): React.ReactElement<Props> {
  ...
  return (
    <main className={className}>
      <Tabs basePath={basePath} items={items.current} />
      <Switch>
        <Route path={`${basePath}/auctions`}>
          <Auctions auctionInfo={auctionInfo} campaigns={campaigns} ownedIds={ownedIds} winningData={winningData} />
        </Route>
        <Route path={`${basePath}/crowdloan`}>
          <Crowdloan auctionInfo={auctionInfo} campaigns={campaigns} leasePeriod={leasePeriod} ownedIds={ownedIds} />
        </Route>
