...
function InputOwner({ onChange, ownedIds }: Props): React.ReactElement<Props> {
  ...
  const owners = useMemo(() => ownedIds.map(({ manager }) => manager), [ownedIds]);
  ...
  return (
    <Modal.Columns
      hint={
        <>
          <p>
            {t<string>("This account that has been used to register the parachain. This will pay all associated fees.")}
          </p>
