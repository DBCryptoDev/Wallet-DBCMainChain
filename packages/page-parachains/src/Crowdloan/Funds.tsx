...
function Funds({ bestNumber, className, leasePeriod, value }: Props): React.ReactElement<Props> {
  ...
  const [active, ended] = useMemo(() => extractLists(value, leasePeriod), [leasePeriod, value]);
  ...
  return (
    <>
      ...
      >
        {active?.map((fund) => (
          <Fund bestNumber={bestNumber} isOngoing key={fund.accountId} value={fund} />
        ))}
      </Table>
      <Table
        className={className}
      ...
      >
        {ended?.map((fund) => (
          <Fund bestNumber={bestNumber} key={fund.accountId} leasePeriod={leasePeriod} value={fund} />
        ))}
      </Table>
    </>
  );
