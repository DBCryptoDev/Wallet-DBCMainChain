...
function Fund({
  ...
  value: {
    childKey,
    info: { cap, depositor, end, firstSlot, lastSlot, raised, retiring },
    isCapped,
    isEnded,
    isRetired,
    isWinner,
  ...
  const trigger = useEventTrigger(
    [api.events.crowdloan.Contributed, api.events.crowdloan.Withdrew],
    useCallback(
      ({
        event: {
          data: [, fundIndex],
        },
      }: EventRecord) => (fundIndex as ParaId).eq(paraId),
      [paraId]
    )
  );
  ...
  useEffect((): void => {
    trigger &&
      api.rpc.childstate
        .getKeys(childKey, "0x")
        .then((keys) => setContributors(extractContributors(allAccounts, keys)))
        .catch(console.error);
  ...
  const [blocksLeft, retiringLeft] = useMemo(
    () =>
      bestNumber
        ? [
            end.gt(bestNumber) ? end.sub(bestNumber) : null,
            retireEnd?.gt(bestNumber) ? retireEnd.sub(bestNumber) : null,
          ]
        : [null, null],
  ...
  const percentage = useMemo(
    () => (cap.isZero() ? "100.00%" : `${(raised.muln(10000).div(cap).toNumber() / 100).toFixed(2)}%`),
    [cap, raised]
  );

  const isLeaseOver =
    !!leasePeriod && (isWinner ? leasePeriod.currentPeriod.gt(lastSlot) : leasePeriod.currentPeriod.gt(firstSlot));
  ...
  return (
    <tr className={className}>
      ...
      <td className="address media--1400">
        <AddressMini value={depositor} />
      </td>
      {!isOngoing && (
        <td className="all number together">
          {(isRetired || retiring.isTrue) && (
            <>
              {retiringLeft && <BlockToTime value={retiringLeft} />}#{formatNumber(retireEnd)}
            </>
          )}
        </td>
      )}
      <td className={`all number together${isOngoing ? "" : " media--1200"}`}>
        {blocksLeft && <BlockToTime value={blocksLeft} />}#{formatNumber(end)}
      </td>
      <td className="number">
        <Digits value={`${formatNumber(firstSlot)} - ${formatNumber(lastSlot)}`} />
      </td>
      <td className="number together">
        <FormatBalance value={raised} withCurrency={false} />
        &nbsp;/&nbsp;
        <FormatBalance value={cap} />
        <div>{percentage}</div>
      </td>
      <td className="number media--1100">{uniqueKeys.length !== 0 && formatNumber(uniqueKeys.length)}</td>
      <td className="badge">
        <Icon color={myAccounts.length ? "green" : "gray"} icon="asterisk" />
      </td>
      <td className="button">
        {canWithdraw && uniqueKeys.length !== 0 && (
          <Withdraw allAccounts={uniqueKeys} myAccounts={myAccounts} paraId={paraId} />
        )}
        {canDissolve && (
          <TxButton
            accountId={depositor}
            ...
            label={isEnded ? t<string>("Dissolve") : t<string>("Cancel")}
            params={[paraId]}
            tx={api.tx.crowdloan.dissolve}
          />
        )}
        {isOngoing && canContribute && <Contribute cap={cap} paraId={paraId} raised={raised} />}
      </td>
    </tr>
  );
}
