...
function createExtrinsics(
  ...
  return addresses
    .reduce(
      (batches: SubmittableExtrinsic<"promise">[][], address): SubmittableExtrinsic<"promise">[][] => {
...
function Withdraw({ allAccounts, className, myAccounts, paraId }: Props): React.ReactElement<Props> {
  ...
  useEffect((): void => {
    const addresses = withdrawType === 0 ? allAccounts : myAccounts;

    console.log("batchSize=", batchSize);

    setExtrinsics(() => (batchSize && addresses.length ? createExtrinsics(api, allAccounts, paraId, batchSize) : null));
  }, [api, allAccounts, batchSize, myAccounts, paraId, withdrawType]);

  return (
    <>
      <Button icon="minus" isDisabled={!hasAccounts} label={t<string>("Withdraw")} onClick={toggleOpen} />
      {isOpen && (
        <Modal className={className} header={t<string>("Withdraw from fund")} size="large">
          <Modal.Content>
