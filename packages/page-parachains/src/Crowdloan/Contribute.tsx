...
function Contribute({ cap, className, paraId, raised }: Props): React.ReactElement<Props> {
  ...
  return (
    <>
      <Button icon="plus" isDisabled={!hasAccounts} label={t<string>("Contribute")} onClick={toggleOpen} />
      {isOpen && (
        <Modal className={className} header={t<string>("Contribute to fund")} size="large">
          <Modal.Content>
            ...
            <Modal.Columns hint={t<string>("The amount to contribute from this account.")}>
              ...
              {isAmountBelow && (
                <MarkWarning
                  content={t<string>("The amount is less than the minimum allowed contribution of {{value}}", {
                    replace: { value: formatBalance(api.consts.crowdloan.minContribution as BlockNumber) },
                  })}
                />
              )}
              {isAmountOver && (
                <MarkWarning
                  content={t<string>("The amount is more than the remaining contribution needed {{value}}", {
                    replace: { value: formatBalance(remaining) },
                  })}
                />
              )}
            </Modal.Columns>
            ...
            >
              ...
              />
              <InputBalance defaultValue={remaining} isDisabled label={t<string>("remaining till cap")} />
            </Modal.Columns>
          </Modal.Content>
