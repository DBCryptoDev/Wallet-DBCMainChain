...
function Proposals({ proposals }: Props): React.ReactElement<Props> {
  ...
  return (
    <Table empty={proposals && sortedIds && t<string>("There are no pending proposals")} header={headerRef.current}>
      {proposals &&
        sortedIds?.map(
          (id): React.ReactNode => (
            <Proposal approvedIds={proposals.approvedIds} id={id} key={id.toString()} scheduled={proposals.scheduled} />
          )
        )}
    </Table>
  );
