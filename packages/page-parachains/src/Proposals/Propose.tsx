...
function Validator({ address, index, setAddress, t }: ValidatorProps): React.ReactElement<ValidatorProps> {
  const _setAddress = useCallback((value: string | null) => value && setAddress(index, value), [index, setAddress]);
...
function Propose({ className, onClose }: Props): React.ReactElement<Props> {
  ...
  const _setGenesisState = useCallback((data: Uint8Array) => setGenesisState(compactAddLength(data)), []);

  const _setWasm = useCallback((wasm: Uint8Array, isWasmValid: boolean) => setWasm({ isWasmValid, wasm }), []);

  const _setAddress = useCallback(
    (index: number, address: string) => setValidators((v) => v.map((v, i) => (i === index ? address : v))),
    []
  );
  ...
  return (
    <Modal className={className} header={t<string>("Propose parachain")} size="large">
      <Modal.Content>
        <Modal.Columns hint={t<string>("This account will be associated with the parachain and pay the deposit.")}>
          <InputAddress label={t<string>("propose from")} onChange={setAccountId} type="account" value={accountId} />
        </Modal.Columns>
        <Modal.Columns hint={t<string>("The name for this parachain, the id and the allocated/requested balance.")}>
          <Input autoFocus isError={!isNameValid} label={t<string>("parachain name")} onChange={setName} />
          <InputNumber isZeroable={false} label={t<string>("requested id")} onChange={setParaId} />
          <InputBalance defaultValue={balance} label={t<string>("initial balance")} onChange={setBalance} />
        </Modal.Columns>
        <Modal.Columns
          hint={t<string>("The WASM validation function as well as the genesis state for this parachain.")}
        >
          <InputWasm
        ...
        >
          {validators.map((address, index) => (
            <Validator address={address} index={index} key={index} setAddress={_setAddress} t={t} />
          ))}
          {!validators.length && (
            <MarkWarning
              content={t<string>(
          ...
          <Button.Group>
            <Button icon="plus" label={t<string>("Add validator")} onClick={_addValidator} />
            <Button
              icon="minus"
