...
function extractScheduledIds(scheduled: ScheduledProposals[] = []): Record<string, boolean> {
  return scheduled.reduce(
    (all: Record<string, boolean>, { scheduledIds }: ScheduledProposals): Record<string, boolean> =>
      scheduledIds.reduce(
...
function mapValidators(
  ...
  validatorGroups?: ParaValidatorIndex[][] | null
): AccountId[][] {
  return validators && validatorGroups && ids && ids.length === validatorGroups.length
    ? validatorGroups.map((ids) => ids.map((id) => validators[id.toNumber()]).filter((a) => a))
    : [];
}
...
function extractEvents(api: ApiPromise, lastBlock: SignedBlockExtended, prev: LastEvents): LastEvents {
  ...
  return wasBacked || wasIncluded || wasTimeout
    ? {
        lastBacked: wasBacked ? { ...prev.lastBacked, ...backed } : prev.lastBacked,
        lastIncluded: wasIncluded ? { ...prev.lastIncluded, ...included } : prev.lastIncluded,
        lastTimeout: wasTimeout ? { ...prev.lastTimeout, ...timeout } : prev.lastTimeout,
      }
    : prev;
}
...
function extractActions(
  actionsQueue: QueuedAction[],
  knownIds?: [ParaId, string][]
): Record<string, QueuedAction | undefined> {
  return actionsQueue && knownIds
    ? knownIds.reduce(
        (all: Record<string, QueuedAction | undefined>, [id, key]) => ({
          ...all,
...
function Parachains({ actionsQueue, ids, scheduled }: Props): React.ReactElement<Props> {
  ...
  const [validators, validatorGroups] = useCallMulti<[AccountId[] | null, ParaValidatorIndex[][] | null]>(
    [api.query.session?.validators, api.query.paraScheduler?.validatorGroups || api.query.scheduler?.validatorGroups],
    optionsMulti
  );
  ...
  const knownIds = useMemo(() => ids?.map((id): [ParaId, string] => [id, id.toString()]), [ids]);
  ...
  return (
    <Table empty={knownIds && t<string>("There are no registered parachains")} header={headerRef.current}>
      {knownIds?.map(
        ([id, key], index): React.ReactNode => (
          <Parachain
