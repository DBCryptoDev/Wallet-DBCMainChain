...
function Upcomings({ actionsQueue, ids, leasePeriod }: Props): React.ReactElement<Props> {
  ...
  return (
    <Table empty={leasePeriod && ids && t<string>("There are no upcoming parachains")} header={headerRef.current}>
      {leasePeriod &&
        ids?.map(
          (id): React.ReactNode => (
            <Upcoming
              ...
              key={id.toString()}
              leasePeriod={leasePeriod}
              nextAction={actionsQueue.find(({ paraIds }) => paraIds.some((p) => p.eq(id)))}
            />
          )
        )}
    </Table>
