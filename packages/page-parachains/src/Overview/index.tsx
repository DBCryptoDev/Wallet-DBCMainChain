...
function Overview({ className, leasePeriod, proposals }: Props): React.ReactElement<Props> {
  ...
  return (
    <div className={className}>
      ...
      {api.query.paras && (
        <>
          <Parachains actionsQueue={actionsQueue} ids={paraIds} scheduled={proposals?.scheduled} />
          <Upcomings actionsQueue={actionsQueue} ids={upcomingIds} leasePeriod={leasePeriod} />
        </>
      )}
    </div>
  );
