...
function renderAddresses(list?: AccountId[]): JSX.Element[] | undefined {
  return list?.map((id) => <AddressMini key={id.toString()} value={id} />);
}
...
function Parachain({
  ...
  const valRender = useCallback(() => renderAddresses(validators), [validators]);
  ...
  useEffect((): void => {
    if (sessionValidators) {
      if (paraInfo.pendingAvail) {
        const list = paraInfo.pendingAvail.availabilityVotes
          .toHuman()
          .slice(2)
          .replace(/_/g, "")
          .split("")
  ...
  return (
    <tr className={className}>
      ...
      <td className="start media--1100">
        <Lifecycle lifecycle={paraInfo.lifecycle} nextAction={nextAction} />
      </td>
      <td className="all" />
      ...
      <td className="number no-pad-left">
        {lastBacked && <a href={`#/explorer/query/${lastBacked.blockHash}`}>{formatNumber(lastBacked.blockNumber)}</a>}
      </td>
      <td className="number no-pad-left">
        {lastTimeout && (
      ...
      <td className="number media--1300">
        {paraInfo.updateAt && bestNumber && (
          <>
            <BlockToTime value={bestNumber.sub(paraInfo.updateAt)} />#{formatNumber(paraInfo.updateAt)}
          </>
        )}
      </td>
      <td className="number media--1200">
        {formatNumber(paraInfo.qUmp)}&nbsp;/&nbsp;{formatNumber(paraInfo.qDmp)}&nbsp;/&nbsp;
        {formatNumber(paraInfo.qHrmpE)}&nbsp;/&nbsp;{formatNumber(paraInfo.qHrmpI)}
      </td>
    </tr>
  );
}
