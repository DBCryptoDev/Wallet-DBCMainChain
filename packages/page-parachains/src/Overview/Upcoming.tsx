...
const optMulti = {
  ...
  transform: ([optHead, optGenesis, optLifecycle, optInfo, leases]: [
    Option<HeadData>,
...
function Upcoming({ id, leasePeriod, nextAction }: Props): React.ReactElement<Props> {
  ...
  return (
    <tr>
      ...
      <td className="start">
        <Lifecycle lifecycle={lifecycle} nextAction={nextAction} />
      </td>
        ...
        {leasePeriod &&
          ...
          (leases.length ? (
            <>
              ...
              <LeaseBlocks leasePeriod={leasePeriod} value={leases[0]?.period} />
            </>
          ) : (
            t("None")
          ))}
