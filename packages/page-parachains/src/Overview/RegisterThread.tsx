...
function RegisterThread({ className, onClose }: Props): React.ReactElement<Props> {
  ...
  return (
    <Modal className={className} header={t<string>("Register parathread")} size="large">
      <Modal.Content>
        <Modal.Columns hint={t<string>("This account will be associated with the parachain and pay the deposit.")}>
          <InputAddress label={t<string>("register from")} onChange={setAccountId} type="account" value={accountId} />
        </Modal.Columns>
        <Modal.Columns hint={t<string>("The id of this parachain as known on the network")}>
          <InputNumber autoFocus isZeroable={false} label={t<string>("parachain id")} onChange={setParaId} />
        </Modal.Columns>
