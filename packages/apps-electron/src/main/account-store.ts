...
export const accountStoreIpcHandler = (fileStore: FileStore): IpcMainHandler => ({
  ...
  "account-store-get": async (key: string) =>
    new Promise((resolve) => {
      try {
  ...
  "account-store-remove": async (key: string) =>
    new Promise((resolve) => fileStore.remove(safeWriteKey(key), () => resolve(undefined))),
  "account-store-set": async (key: string, value: KeyringJson) =>
    new Promise((resolve) => fileStore.set(safeWriteKey(key), value, () => resolve(undefined))),
});
