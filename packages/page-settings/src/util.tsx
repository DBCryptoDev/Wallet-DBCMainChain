...
export function createOption(
  { info, isHeader, text, value }: Option,
  overrides: string[] = [],
  override = "empty"
  ...
  return {
    text: (
      <div className="ui--Dropdown-item" key={value}>
        <ChainImg className="ui--Dropdown-icon" logo={info && overrides.includes(info) ? override : info} />
...
export function createIdenticon({ info, text, value }: Option, overrides: string[] = [], override = "empty"): Option {
  const theme = info && overrides.includes(info) ? (override as "empty") : (info as "substrate");

  return {
    text: (
      <div className="ui--Dropdown-item" key={value}>
        {theme === "empty" ? (
          <ChainImg className="ui--Dropdown-icon" logo="empty" />
        ) : (
          <IdentityIcon
