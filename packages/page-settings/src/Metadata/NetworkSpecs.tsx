...
function NetworkSpecs({ chainInfo, className }: Props): React.ReactElement<Props> {
  ...
  const _onChangeColor = useCallback((color: string): void => setNetworkSpecs({ color }), []);
  const _onSetRandomColor = useCallback((): void => setNetworkSpecs({ color: getRandomColor() }), []);
  const _checkColorValid = useCallback(
    (): boolean => /^#[\da-fA-F]{6}|#[\da-fA-F]{3}$/.test(networkSpecs.color),
    [networkSpecs]
  );

  const headerRef = useRef([[t("chain specificiations"), "start", "2"]]);
  ...
  return (
    <Table className={className} empty={t<string>("No open tips")} header={headerRef.current}>
      <tr>
        <td>
          ...
          <div className="settings--networkSpecs-color">
            <div>
              <Input
                ...
                help={t<string>(
                  'The color used to distinguish this network with others, use color code with 3 or 6 digits, like "#FFF" or "#111111"'
                )}
            ...
            <ChainColorIndicator className="settings--networkSpecs-colorBar" color={networkSpecs.color} />
          </div>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td>
          <Input
            className="full"
            help={t<string>(
              "Prefix indicates the ss58 address format in this network, it is a number between 0 ~ 255 that describes the precise format of the bytes of the address"
            )}
            isDisabled
