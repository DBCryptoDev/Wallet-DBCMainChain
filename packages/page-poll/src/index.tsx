...
function PollApp({ basePath, className }: Props): React.ReactElement<Props> {
  ...
  const [totalIssuance, totals] = useCallMulti<MultiResult>(
    [api.query.balances.totalIssuance, api.query.poll.totals],
    optMulti
  );
  ...
  const options: [string, string, boolean, (value: boolean) => void][] = [
    [
      t("No change"),
      t("No change from the original 2017 sale definitions; will mean a total of 10 million DOT from genesis."),
      opt10m,
      setOpt10m,
    ],
    [
      ...
      <div className="pollContainer">
        <div className="pollHeader">
          ...
          <div className="pollBlocksRight">
            ...
            <div>
              {canVote ? <BlockToTime value={blocksLeft} /> : t<string>("Completed")}
        ...
        <article className="keepAlive">
          <p>
            <Trans key="poll1">
              The Polkadot DOT denomination vote: Seventy-two hours after the DOT token becomes transferable, the most
              popular option from this poll will decide the denomination used for the DOT token.
            </Trans>
          </p>
          <p>
            <Trans key="poll2">
              This is an{" "}
              <a href="https://en.wikipedia.org/wiki/Approval_voting" rel="noreferrer" target="_blank">
                approval vote
              </a>
              . There are four options and you may select any combination of them. The most popular of the four will be
              selected as the final DOT denomination three days after DOT token transfers are enabled.
            </Trans>
          </p>
          <p>
            <Trans key="poll3">
              Please see the{" "}
              <a
                href="https://medium.com/polkadot-network/the-first-polkadot-vote-1fc1b8bd357b"
                rel="noreferrer"
                target="_blank"
              >
                Medium article{" "}
                ...
                <Columar.Column>
                  {totals[index].isZero() ? (
                    ...
                    <div className="result">
                      ...
                      <Progress isDisabled={!turnout} total={turnout?.voted} value={totals[index]} />
                    </div>
                  )}
                </Columar.Column>
              </Columar>
          ...
          {canVote && (
            <>
              <InputAddress label={t("vote using my account")} onChange={setAccountId} type="account" />
              <Button.Group>
                <TxButton
                  accountId={accountId}
                  icon="paper-plane"
        ...
        <div className="pollActions">
          <ul>
            ...
            <li>
              {t("Approving of all or none of the options is equivalent and will not affect the outcome of the poll.")}
            </li>
