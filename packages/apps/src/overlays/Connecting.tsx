...
function Connecting({ className }: Props): React.ReactElement<Props> | null {
  ...
  if (apiError) {
    return (
      <BaseOverlay className={className} icon="globe" type="error">
        <div>{apiError}</div>
      </BaseOverlay>
    );
  } else if (!isApiReady) {
    return (
      <BaseOverlay className={className} icon="globe" type="info">
    ...
    return (
      <BaseOverlay className={className} icon="puzzle-piece" type="info">
        <div>
          {t<string>(
