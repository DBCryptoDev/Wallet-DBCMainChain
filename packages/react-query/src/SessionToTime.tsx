  ...
  const blocks = useMemo(
    () =>
      sessionInfo && value && sessionInfo.currentIndex.lt(value)
        ? value.sub(sessionInfo.currentIndex).imul(sessionInfo.sessionLength).isub(sessionInfo.sessionProgress)
        : BN_ZERO,
    [sessionInfo, value]
  );

  return (
    <BlockToTime className={className} isInline={isInline} label={label} value={blocks}>
      {children}
    </BlockToTime>
  );
}
