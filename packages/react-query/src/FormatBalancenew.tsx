...
function getFormat(registry: Registry, formatIndex = 0): [number, string] {
  ...
  return [
    ...
    formatIndex < tokens.length ? tokens[formatIndex] : tokens[0],
  ];
}
...
function FormatBalancenew({
  ...
  const formatInfo = useMemo(() => getFormat(api.registry, formatIndex), [api, formatIndex]);
...
export default React.memo(styled(FormatBalancenew)`
