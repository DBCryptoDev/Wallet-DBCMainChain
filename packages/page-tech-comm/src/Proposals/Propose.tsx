...
function Propose({ isMember, members }: Props): React.ReactElement<Props> {
  ...
  const _hasThreshold = useCallback(
    (threshold?: BN | null): boolean => !!threshold && !threshold.isZero() && threshold.lten(members.length),
    [members]
  );

  const _onChangeExtrinsic = useCallback(
    (proposal?: SubmittableExtrinsic<"promise">): void =>
      setProposal({
        proposal,
  ...
  return (
    <>
      {isOpen && (
        <Modal header={t<string>("Propose a committee motion")} onClose={onClose}>
          <Modal.Content>
            <InputAddress
              filter={members}
            ...
            />
            <Extrinsic defaultValue={apiDefaultTxSudo} label={t<string>("proposal")} onChange={_onChangeExtrinsic} />
          </Modal.Content>
      ...
      <Button icon="plus" isDisabled={!isMember} label={t<string>("Submit proposal")} onClick={onOpen} />
    </>
  );
}
