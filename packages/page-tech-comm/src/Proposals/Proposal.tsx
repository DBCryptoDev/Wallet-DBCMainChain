...
function Proposal({ className = "", imageHash, members, prime }: Props): React.ReactElement<Props> | null {
  ...
  const proposal = useCall<ProposalType | null>(
  ...
  const { hasFailed, isCloseable, isVoteable, remainingBlocks } = useVotingStatus(
    votes,
    members.length,
    "technicalCommittee"
  );
  ...
  return (
    <tr className={className}>
      ...
      <ProposalCell imageHash={imageHash} proposal={proposal} />
      <td className="number">
      ...
      <td className="address">
        {ayes.map(
          (address, index): React.ReactNode => (
            <AddressMini key={`${index}:${address.toHex()}`} value={address} withBalance={false} />
          )
        )}
      </td>
      <td className="address">
      ...
      <td className="button">
        {isVoteable && !isCloseable && <Voting hash={imageHash} members={members} prime={prime} proposalId={index} />}
        {isCloseable &&
          (isMultiMembers ? (
            <Close hasFailed={hasFailed} hash={imageHash} idNumber={index} members={members} proposal={proposal} />
          ) : (
            <TxButton
              accountId={councilId}
              icon="times"
