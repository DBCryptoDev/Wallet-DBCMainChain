...
function findAvailbleId(assetIds: AssetId[]): BN {
  return assetIds.length ? assetIds[assetIds.length - 1].add(BN_ONE) : BN_ONE;
}
...
function Info({ assetIds, className = "", defaultValue, onChange }: Props): React.ReactElement<Props> {
  ...
  return (
    <Modal.Content className={className}>
      <Modal.Columns
        hint={t<string>("The account that is to be used to create this asset and setup the initial metadata.")}
      >
        <InputAddress
      ...
      <Modal.Columns
        hint={t<string>("The selected id for the asset. This should not match an already-existing asset id.")}
      >
        <InputNumber
          autoFocus
