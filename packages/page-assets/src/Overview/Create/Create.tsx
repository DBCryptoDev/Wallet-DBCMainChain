...
function Create({ assetIds, className, onClose }: Props): React.ReactElement<Props> {
  ...
  return (
    <Modal
      ...
      size="large"
    >
      {step === 1 && <Info assetIds={assetIds} defaultValue={asset} onChange={setAsset} />}
      {step === 2 && <Metadata assetId={asset?.assetId} defaultValue={metadata} onChange={setMetadata} />}
      {step === 3 && (
        <Team accountId={asset?.accountId} assetId={asset?.assetId} defaultValue={team} onChange={setTeam} />
      )}
      <Modal.Actions onCancel={onClose}>
        {step === 1 && <Button icon="step-forward" isDisabled={!asset} label={t<string>("Next")} onClick={nextStep} />}
        {step === 2 && (
          <>
            <Button icon="step-backward" label={t<string>("Prev")} onClick={prevStep} />
            <Button icon="step-forward" isDisabled={!metadata} label={t<string>("Next")} onClick={nextStep} />
          </>
        )}
        {step === 3 && (
          <>
