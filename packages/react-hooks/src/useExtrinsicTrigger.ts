...
export function useExtrinsicTrigger(checks: ExtrinsicCheck[]): string {
  ...
  useEffect((): void => {
    mountedRef.current &&
      block &&
      block.extrinsics &&
      block.extrinsics.filter(
        ({ extrinsic }) =>
          extrinsic &&
          checks.some((check) => check && (isString(check) ? extrinsic.method.section === check : check.is(extrinsic)))
      ).length &&
      setTrigger(() => block.createdAtHash?.toHex() || "0x00");
