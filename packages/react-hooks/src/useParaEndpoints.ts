...
function extractResults(genesisHash: string, paraId: BN | number): LinkOption[] {
  ...
  return createWsEndpoints((key: string, value: string | undefined) => value || key).filter(
    ({ genesisHashRelay, paraId }) => paraId === numId && genesisHashRelay === genesisHash
  );
}
...
export function useParaEndpoints(paraId: BN | number): LinkOption[] {
  ...
  return useMemo(() => extractResults(api.genesisHash.toHex(), paraId), [api, paraId]);
}
