...
function writeFiles(name, content) {
  [DST, SRC].forEach((root) => fs.writeFileSync(`${root}/ipfs/${name}`, content, WOPTS));
}
...
async function unpin(exclude) {
  ...
  if (result.count > 1) {
    const filtered = result.rows.map((r) => r.ipfs_pin_hash).filter((hash) => hash !== exclude);
...
async function dnslink(hash) {
  const records = createWsEndpoints(() => "")
    ...
    .map((sub) => ["_dnslink", sub, DOMAIN].filter((entry) => !!entry).join("."));

  await Promise.all(
    records.map((record) =>
      cloudflare({ token: process.env.CF_API_TOKEN }, { link: `/ipfs/${hash}`, record, zone: DOMAIN })
    )
  );
